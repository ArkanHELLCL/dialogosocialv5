...
<%	
	...
	if(mode="mod") then
		set rs = cnn.Execute("exec spTematica_Consultar " & TEM_Id)
		on error resume next	
		cnn.open session("DSN_DialogoSocialv5")
		if cnn.Errors.Count > 0 then 
		   ErrMsg = cnn.Errors(0).description	   
		...
		if not rs.eof then
			...
			TEM_Id      = rs("TEM_Id")
			TEM_Nombre  = rs("TEM_Nombre")
			LIN_Estado  = rs("LIN_Estado")
			TEM_Horas	= rs("TEM_Horas")
		end if		
		rs.Close		
	end if
	...
	<div class="modal-dialog cascading-modal narrower modal-xl" role="document">  		
		<div class="modal-content">		
			<div class="modal-header view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 d-flex justify-content-center align-items-center" style="height: 3rem;">
				<div class="text-left" style="font-size:1.25rem;"><i class="fas fa-server"></i> Módulos</div>				
			</div>				
			<div class="modal-body" style="padding:0px;">
				<div id="divfrmmodulos" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					<div class="px-4">						
						<form role="form" action="<%=action%>" method="POST" name="<%=frmmodulos%>" id="<%=frmmodulos%>" class="needs-validation">
							<div class="row">
								...
								<div class="col-sm-12 col-md-12 col-lg-6">
									<div class="md-form input-with-post-icon">
										<div class="error-message">
											<div class="select"><%
												if(mode="add") then%>
													<select name="MOD_Id" id="MOD_Id" class="select-text form-control" required><%
												else%>
													<select name="MOD_Id" id="MOD_Id" class="select-text form-control" disabled><%
												end if												
												...
												<span class="select-bar"></span><%
												if(mode="add") then%>
													<label class="select-label">Cursos</label><%
												else%>
													<label class="select-label active">Cursos</label><%
												end if%>
											</div>
										</div>
									</div>
							...
							<div class="row">
								...
								<div class="col-sm-12 col-md-12 col-lg-6">
									<div class="md-form input-with-post-icon">
										<div class="error-message">
											<div class="select">
												<select name="PER_Id" id="PER_Id" class="select-text form-control" <%=ds%>><%
												...
												<span class="select-highlight"></span>
												<span class="select-bar"></span>
												<label class="select-label <%=lblSelect%>">Perspectivas</label>
											</div>
										</div>
									</div>
								</div>																						
								...
								<div class="col-sm-12 col-md-12 col-lg-6">
									<div class="md-form input-with-post-icon">
										<div class="error-message">								
											...
											<input type="text" id="TEM_Nombre" name="TEM_Nombre" class="form-control" <%=disabled%> value="<%=TEM_Nombre%>">
											<span class="select-bar"></span>
											<label for="TEM_Nombre" class="<%=lblClass%>">Módulo</label>
										</div>
									</div>
								</div>
							</div>
								...
								<div class="col-sm-12 col-md-12 col-lg-4">
									<div class="md-form input-with-post-icon">
										<div class="error-message">								
											...
											<input type="number" id="TEM_Horas" name="TEM_Horas" class="form-control" <%=disabled%> value="<%=TEM_Horas%>">
											<span class="select-bar"></span>
											<label for="TEM_Horas" class="<%=lblClass%>">Horas pedagógicas mínimas</label>
										</div>
									</div>
								</div>
							</div><%
							if(mode="mod") then%>
								<input type="hidden" id="TEM_Id" name="TEM_Id" value="<%=TEM_Id%>"><%
							end if%>						
						</form>
						<!--form-->
					</div>
			...
			<div class="modal-footer"><%
				if (session("ds5_usrperfil")=1) then%>
					<div style="float:left;" class="btn-group" role="group" aria-label="">
						<button class="<%=button%>" type="button" data-url="" title="Modificar Linea Formativa" id="btn_frmmodulos" name="btn_frmmodulos"><%=typeFrm%></button>
					</div><%
				end if%>
...
<script>    
	...
	$(document).ready(function() {
		...
		function perspectivas(PER_Id){
			var MOD_Id = $("#MOD_Id").val();			
			$.ajax({
				type: 'POST',
				url: '/seleccionar-perspectivas',
				data: {MOD_Id:MOD_Id,PER_Id:PER_Id},
				success: function(data) {						
					$("#PER_Id").html(data);
				},
				error: function(XMLHttpRequest, textStatus, errorThrown){						
		...
		$("#btn_frmmodulos").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			var mode='<%=mode%>';			
			...
			formValidate("#frmmodulos");			
			if($("#frmmodulos").valid()){				
				$.ajax({
					type: 'POST',
					url: $("#frmmodulos").attr("action"),
					data: $("#frmmodulos").serialize(),
					dataType: "json",
					success: function(data) {						
						if(data.state=="200"){
							if(mode=="add"){
