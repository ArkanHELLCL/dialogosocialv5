				...
				<div class="view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center" style="height:3rem;">
					<div><%
						if (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2) then%>
							<button class="btn btn-success btn-rounded btn-sm waves-effect" title="Crear un nuevo festivo" type="button" data-url="" data-toggle="tooltip" data-id="10" id="btn_addfestivosModal" name="btn_addfestivosModal">Agregar<i class="fas fa-plus ml-1"></i></button><%
						end if%>
					</div>
...
<script>
	$(document).ready(function(e) {
					...
					$(nRow).click(function(e){						
						...
						$.ajax( {
							...
							success: function ( data ) {
								param = data.split(bb)
								if(param[0]==200){							
									$("#festivosModal").html(param[1]);
									$("#festivosModal").modal("show");
								}else{
		...
		$("#festivosModal").on('hidden.bs.modal', function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation()
			tablefestivo();
		...
		$("#btn_addfestivosModal").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();						
			$.ajax( {
