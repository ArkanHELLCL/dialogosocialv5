...
<%		
	...
	set rs = cnn.Execute("exec spInformeMesasEmpresas_Listar")
	on error resume next
	if cnn.Errors.Count > 0 then 
		ErrMsg = cnn.Errors(0).description
		response.write("Error spInformeMesasEmpresas_Listar")
		cnn.close 		
		response.end
	End If	
	if not rs.eof then
		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">			
			...
			<div class="card card-cascade narrower">
				...
				<div class="view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center" style="height:3rem;">
					...
					<a href="" class="white-text mx-3"><i class="fas fa-file-invoice"></i> Informe de Organizaciones Empresariales</a>
					<div>						
						<button class="btn btn-secondary btn-rounded buttonExport btn-sm waves-effect" data-toggle="tooltip" title="Exportar datos de la tabla" data-id="orgempresas">Exportar<i class="fas fa-download ml-1"></i></button>
					</div>
				</div>
				<!--/Card image-->
					...
					<div class="table-wrapper col-sm-12" style="overflow-x:auto">
						<!--Table-->
						<table id="tbl-orgempresas" class="table-striped table-bordered dataTable table-sm" cellspacing="0" width="100%" data-id="orgempresas" >
							<thead>
								<tr> 
									<th>Id Proyecto</th>                         
									...
									<th>Id Organizacion Empresa</th>
									<th>Nombre Organizacion Empresa</th>									
									<th>ROL</th>
									<th>Id Rubro</th>
									<th>Rubro</th>									
									<th>Telefono</th>
...
<script>
	$(document).ready(function(e) {		
		...
		$('#tbl-orgempresas tbody').on( 'click', 'td', function (e) {
			e.stopImmediatePropagation();
			e.stopPropagation();
		} );							
		function tableorgempresas(){			
			var tables = $.fn.dataTable.fnTables(true);
			$(tables).each(function () {
				$(this).dataTable().fnDestroy();
			});
			orgempresasTable = $('#tbl-orgempresas').DataTable({
				lengthMenu: [ 10,15,20 ],
				processing: true,
				columnDefs: [
					{ targets: [0,2,4,5,6,9,11,12,13,14,<%=col%>], visible: false},					
				],				
			});
			...
			$('.btn_GenerarInfo').on('click', function() {
				...
				$.ajax({
					url: "/tbl-organizacionesempresas",
					type: "post",					
					dataSrc:function(json){												
						return json.data;
					},					
				}).done(function (result) {										
					orgempresasTable.clear().draw();
					orgempresasTable.rows.add(JSON.parse(result).data).draw();										
