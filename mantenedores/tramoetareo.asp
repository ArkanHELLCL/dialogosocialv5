				...
				<div class="view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center" style="height:3rem;">
					<div><%
						if (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2) then%>
							<button class="btn btn-success btn-rounded btn-sm waves-effect" title="Crear un nuevo festivo" type="button" data-url="" data-toggle="tooltip" data-id="10" id="btn_addetareoModal" name="btn_addetareoModal">Agregar<i class="fas fa-plus ml-1"></i></button><%
						end if%>
					</div>
					<a href="" class="white-text mx-3"><i class="fas fa-server"></i> Mantenedor de Tramos Etarios</a>
					<div>						
...
<script>
	$(document).ready(function(e) {
			...
			festivoTable = $('#tbl-festivo').DataTable({
				...
				"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {					
					$(nRow).click(function(e){						
						...
						var TRE_Id = $(this).find("td")[0].innerText;
						var data={TRE_Id:TRE_Id,mode:'mod'}
						
						$.ajax( {
							type:'POST',
							url: '/modal-tramo-etareo',
							data: data,
							success: function ( data ) {
								param = data.split(bb)
								if(param[0]==200){							
									$("#etareoModal").html(param[1]);
									$("#etareoModal").modal("show");
								}else{
		...
		$("#etareoModal").on('hidden.bs.modal', function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation()
			tablefestivo();
		...
		$("#btn_addetareoModal").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();						
			$.ajax( {
