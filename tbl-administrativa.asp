...
<%					
	...
	if (session("ds5_usrperfil")<>3) then	'Todos menos el ejecutor		
		sql="exec spProyectoFondos_Listar 1," & FON_Id & "," & session("ds5_usrid") & ",'" & session("ds5_usrtoken") & "'"
	else		
	...
	do While (Not rs.EOF)		
		...
		if(LFO_Id=10) then
			if rs("PRY_CreacionProyectoEstado")=1 then
				if rs("PRY_InformeInicioAceptado")=1 then
					if rs("PRY_InformeParcialAceptado")=1 then						
						if rs("PRY_InformeFinalAceptado")=1 then
							estado="Finalizado"
						else
							estado="Final"
						end if
			...
			if(LFO_Id=12) then
				...
				if(LFO_Id=11) then
					if rs("PRY_CreacionProyectoEstado")=1 then
						if rs("PRY_InformeInicialEstado")=1 then
							if rs("PRY_InformeConsensosEstado")=1 then
								if rs("PRY_InformeSistematizacionEstado")=1 then								
									estado="Cerrado"
								else
		...
		'Presupuesto
		set rsx=cnn.execute("spPresupuesto_Listar " & rs("PRY_Id"))
		on error resume next
		if cnn.Errors.Count > 0 then 
			ErrMsg = cnn.Errors(0).description
			response.write("503/@/Error SQL: " & ErrMsg & "- spPresupuesto_Listar" )
			cnn.close 			   
			Response.end()
		End If		
		portot=0			
		poravance = 0
		poretraso = 0						
		pagocuota_1="Pendiente"
		pagocuota_2="Pendiente"
		pagocuota_3="Pendiente"
		do While Not rsx.EOF
			if(rsx("PRE_NumCuota")=1) then				
				pagocuota_1=rsx("PRE_FechaPagoCuota")
				if(pagocuota_1="") then
					pagocuota_1="Pendiente"					
				end if
			end if
			if(rsx("PRE_NumCuota")=2) then				
				pagocuota_2=rsx("PRE_FechaPagoCuota")
				if(pagocuota_2="") then
					pagocuota_2="Pendiente"					
				end if
			end if
			if(rsx("PRE_NumCuota")=3) then			
				pagocuota_3=rsx("PRE_FechaPagoCuota")
				if(pagocuota_3="") then
					pagocuota_3="Pendiente"					
				end if
			end if
			if(rsx("PRE_EstadoCuota")=1) then
				poravance = poravance + CInt(rsx("PRE_PorcentajeMonto"))
			end if
			rsx.movenext
		loop
		poretraso = 100 - poravance
		if(poravance = 0) then
			poretraso = 0
		end if
		porcentaje = "<span class='ok'>" & poravance & "%</span><span class='nok'>" & poretraso & "%</span></div>"		
				
		dataAdministrativa = dataAdministrativa & "[""" & rs("PRY_Id") & """,""" & rs("PRY_EmpresaEjecutora") & """,""" & PMixto & """,""" & rs("PRY_CodigoAsociado") & """,""" & rs("LFO_Id") & """,""" & rs("LFO_Nombre") & """,""" & rs("LIN_Id") & """,""" & rs("LIN_Nombre") & """,""" & LMixta & """,""" & rs("REG_Id") & """,""" & rs("PRY_EncargadoActividades") & """,""" & rs("PRY_NumResAprueba") & """,""" & rs("PRY_FechaResolucion") & """,""" & rs("PRY_AnioProyecto") & """,""" & fechaPrimerInforme & """,""" & estadoPrimer & """,""" & pagocuota_1 & """,""" & fechaAceptadoPrimerInforme & """,""" & fechaSegundoInforme & """,""" & estadoSegundo & """,""" & pagocuota_2 & """,""" & fechaAceptadoSegundoInforme & """,""" & fechaTercerInforme & """,""" & estadoTercer & """,""" & pagocuota_3 & """,""" & rs("PRY_CierreFecha") & """,""" & fechaAceptadoTercerInforme & """,""" & estado & """,""" & msg & """,""" & rs("REG_Nombre") & """,""" & rs("COM_Nombre") & """,""" & rs("USR_NombreRevisor") & """,""" & rs("USR_ApellidoRevisor") & """,""" & rs("USR_MailRevisor") & """,""" & rs("USR_TelefonoRevisor") & """,""" & rs("USR_DireccionRevisor") & """,""" & rs("USR_NombreEjecutor") & """,""" & rs("USR_ApellidoEjecutor") & """,""" & rs("USR_MailEjecutor") & """,""" & rs("USR_TelefonoEjecutor") & """,""" & rs("USR_DireccionEjecutor") & """,""" & rs("USR_NombreInstitucionEjecutor") & """,""" & USR_SexoEjecutor & """,""" & rs("PRY_EncargadoProyecto") & """,""" & rs("PRY_EncargadoProyectoMail") & """,""" & rs("PRY_EncargadoProyectoCelular") & """,""" & SEX_IdEncargadoProyecto & """,""" & rs("PRY_EncargadoActividadesMail") & """,""" & rs("PRY_EncargadoActividadesCelular") & """,""" & SEX_IdEncargadoActividades & """,""" & rs("PRY_InformeInicioFechaEnvio") & """,""" & rs("PRY_InformeFinalFechaEnvio") & """,""" & rs("PRY_CreacionProyectoFechaEnvio") & """,""" & estadoPrimerInforme & """,""" & estadoSegundoInforme & """,""" & estadoTercerInforme & """,""" & rs("PRY_LanzamientoDireccion") & """,""" & rs("PRY_LanzamientoFecha") & """,""" & rs("PRY_LanzamientoHora") & """,""" & rs("PRY_CierreDireccion") & """,""" & rs("PRY_CierreHora") & """,""" & diferenciaPrimerInforme & """,""" & diferenciaSegundoInforme & """,""" & diferenciaTercerInforme & """,""" & poravance & """,""" & poretraso & """,""" & porcentaje & """]"											
						
		rs.MoveNext
		if not rs.eof then
			dataAdministrativa = dataAdministrativa & ","
		end if
  Loop   	   				
  rs.Close
  cnn.Close     
  ...
  dataAdministrativa=dataAdministrativa & "]}"	
  response.write(dataAdministrativa)	  
%>