...
<%
	...
	set rs = cnn.Execute("exec [spPatrociniosMesas_Listar] " & PRY_Id)
	on error resume next
	if cnn.Errors.Count > 0 then 
		ErrMsg = cnn.Errors(0).description
		response.write("Error [spPatrociniosMesas_Listar] " & ErrMsg)
		cnn.close 		
		response.end
	End If	
	cont=1	
	...
	loop
	dataPatrocinios=dataPatrocinios & "],""VER_TotalMesas"":""" & VER_TotalMesas & """,""VER_EstadoAprobadoMesas"":""" & VER_EstadoAprobadoMesas & """,""VER_EstadoSubidoMesas"":""" & VER_EstadoSubidoMesas & """,""VER_EstadoRechazadoMesas"":""" & VER_EstadoRechazadoMesas & """,""VER_EstadoRevisadoMesas"":""" & VER_EstadoRevisadoMesas & """}"
	set f=nothing
	set fs=nothing
