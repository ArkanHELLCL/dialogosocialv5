...
<form role="form" action="<%=action%>" method="POST" name="frm11s1" id="frm11s1" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosini11" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosini11" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableIni11;		
		loadTableDocumentos();
        $('#tbl-documentosini11').css('width','99%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosini11")){				
				$('#tbl-documentosini11').dataTable().fnClearTable();
    			$('#tbl-documentosini11').dataTable().fnDestroy();
			}	
			
			documentosTableIni11 = $('#tbl-documentosini11').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){
										documentosTableIni11.ajax.reload();
										Toast.fire({
										icon: 'success',
