...
$html = $html.'<table  border="0">
                  <tr>
                    <th scope="col" width="33%">Horas Pegagógicas Mínimas</th>
...
$html = $html.'<table border=0>
                    <tr>
                        <th scope="col" width="50%">Actividad Económica</th>
                        <th scope="col" width="50%">Macrozona</th>
                    </tr>
                    <tr>
                        <td width="50%">'.$RUB_Nombre.'</td>
                        <td width="50%">'.$MCZ_Descripcion.'</td>
                    </tr>
                  </table>';
$html = $html.'<table border=0>
                  <tr>
                      <th scope="col" width="100%">Temática</th>
                  </tr>
                  <tr>
                      <td width="100%">'.$TEM_Descripcion.'</td>
                  </tr>
                </table>';
$html = $html.'<table border=0>
                  <tr>
                      <th scope="col" width="100%">Dimensión del Diálogo Social</th>
                  </tr>
                  <tr>
                      <td width="100%">'.$PRY_DimensionDialogoSocial.'</td>
                  </tr>
                </table>';     
...
$html = $htmlstyle.'<h4>Diagnóstico Social Laboral</h4>    
    <table border="0">
    <tr>
        <th scope="col">Diagnósico Social Laboral</th>
    </tr>
    <tr>
...
$tsql_callSP = "spPatrocinioEmpresa_Listar ?";
$params = array(   
            array($data->PRY_Id, SQLSRV_PARAM_IN)            
    ); 
$stmt = sqlsrv_query( $conn, $tsql_callSP, $params);
...
if($PRY_TipoMesa==2){
    $html = $html.$datos.'</table>';
    $html = $html.'<h5>Organizaciones de Gobierno</h5>
                <table  border="0">
                    <tr>
                        <th scope="col">Organización de Gobierno</th>                    
                        <th scope="col">Compromiso</th>                    
                    </tr>';
    $datos = '<tr>
                <td colspan="3" style="text-align:center">Sin datos</td>
    ...
    $tsql_callSP = "spPatrocinioGobierno_Listar ?";
    $params = array(   
                array($data->PRY_Id, SQLSRV_PARAM_IN)            
        ); 
    $stmt = sqlsrv_query( $conn, $tsql_callSP, $params);
    ...
    {              
        $datosTabla = $datosTabla.'<tr>
                    <td>'.$row['SER_Nombre'].'</td>                
                    <td>'.$row['PGO_Compromiso'].'</td>
                </tr>';
    };
    if(strlen($datosTabla)>0){
        $datos=$datosTabla;
...
$pdf->AddPage('L','A4');
$html = $htmlstyle.'<h4>Representante(s)</h4>
                <h4>Representante(s) Sindicato(s)</h4>';
$x=0;
$tsql_callSP = "spProyectoSindicato_Listar ?, -1";
$params = array(   
            array($data->PRY_Id, SQLSRV_PARAM_IN)            
    ); 
$stmt = sqlsrv_query( $conn, $tsql_callSP, $params);
...
{   
    if($row['Total']>=1){           
        $x=$x+1;
        $html=$html.'<h4>'.$x.'.- '.$row['SIN_Nombre'].'</h4>        
            <table  border="0">
                <tr>
                    <th>Nombre</th>                    
                    <th>Apellio Paterno</th>
                    <th>Apellio Materno</th>
                    <th>Rut</th>
                    <th>Cargo</th>
                    <th>Sexo</th>
                </tr>';
        $tsql_callSP2 = "spRepProyectoSindicato_Listar 1, ?, ?";
        $params2 = array(   
                    array($data->PRY_Id, SQLSRV_PARAM_IN),
                    array($row['SIN_id'], SQLSRV_PARAM_IN)
            ); 
        $stmt2 = sqlsrv_query( $conn, $tsql_callSP2, $params2);
        while( $row2 = sqlsrv_fetch_array( $stmt2, SQLSRV_FETCH_ASSOC))  
        {                      
            $html=$html.'<tr>
                        <td>'.$row2['RPS_Nombre'].'</td>
                        <td>'.$row2['RPS_ApellidoPaterno'].'</td>
                        <td>'.$row2['RPS_ApellidoMaterno'].'</td>
                        <td>'.$row2['RPS_Rut'].'-'.$row2['RPS_DV'].'</td>
                        <td>'.$row2['RPS_Cargo'].'</td>
                        <td>'.$row2['SEX_Descripcion'].'</td>
                        </tr>';
        };
        $html=$html.'</table><br>';
    }
...
$pdf->AddPage('L','A4');
$html = $htmlstyle.'<h4>Representante(s)</h4>
                <h4>Representante(s) Empresa(s)</h4>';
$x=0;
$tsql_callSP = "spProyectoEmpresa_Listar ?, -1";
$params = array(   
            array($data->PRY_Id, SQLSRV_PARAM_IN)            
    ); 
$stmt = sqlsrv_query( $conn, $tsql_callSP, $params);
...
{   
    if($row['Total']>=1){           
        $x=$x+1;
        $html=$html.'<h4>'.$x.'.- '.$row['EMP_Nombre'].'</h4>        
            <table  border="0">
                <tr>
                    <th>Nombre</th>                    
                    <th>Apellio Paterno</th>
        ...
        $tsql_callSP2 = "spRepProyectoEmpresa_Listar 1, ?, ?";
        $params2 = array(   
                    array($data->PRY_Id, SQLSRV_PARAM_IN),
                    array($row['EMP_Id'], SQLSRV_PARAM_IN)
            ); 
        $stmt2 = sqlsrv_query( $conn, $tsql_callSP2, $params2);
        while( $row2 = sqlsrv_fetch_array( $stmt2, SQLSRV_FETCH_ASSOC))  
        {                      
            $html=$html.'<tr>
                        <td>'.$row2['RPE_Nombre'].'</td>
                        <td>'.$row2['RPE_ApellidoPaterno'].'</td>
                        <td>'.$row2['RPE_ApellidoMaterno'].'</td>
                        <td>'.$row2['RPE_Rut'].'-'.$row2['RPE_DV'].'</td>
                        <td>'.$row2['RPE_Cargo'].'</td>
                        <td>'.$row2['SEX_Descripcion'].'</td>
                        </tr>';
        };
        $html=$html.'</table><br>';
...
if($PRY_TipoMesa==2){
    $pdf->AddPage('L','A4');
    $html = $htmlstyle.'<h4>Representante(s)</h4>
                    <h4>Representante(s) Gobierno</h4>';
    $x=0;
    $tsql_callSP = "spProyectoServicio_Listar ?, -1";
    $params = array(   
                array($data->PRY_Id, SQLSRV_PARAM_IN)            
        ); 
    $stmt = sqlsrv_query( $conn, $tsql_callSP, $params);
    ...
    {   
        if($row['Total']>=1){           
            $x=$x+1;
            $html=$html.'<h4>'.$x.'.- '.$row['SER_Nombre'].'</h4>        
                <table  border="0">
                    <tr>
                        <th>Nombre</th>                    
                        <th>Apellio Paterno</th>
            ...
            $tsql_callSP2 = "spRepProyectoGobierno_Listar 1, ?, ?";
            $params2 = array(   
                        array($data->PRY_Id, SQLSRV_PARAM_IN),
                        array($row['SER_Id'], SQLSRV_PARAM_IN)
                ); 
            $stmt2 = sqlsrv_query( $conn, $tsql_callSP2, $params2);
            while( $row2 = sqlsrv_fetch_array( $stmt2, SQLSRV_FETCH_ASSOC))  
            {                      
                $html=$html.'<tr>
                            <td>'.$row2['RPG_Nombre'].'</td>
                            <td>'.$row2['RPG_ApellidoPaterno'].'</td>
                            <td>'.$row2['RPG_ApellidoMaterno'].'</td>
                            <td>'.$row2['RPG_Rut'].'-'.$row2['RPG_DV'].'</td>
                            <td>'.$row2['RPG_Cargo'].'</td>
                            <td>'.$row2['SEX_Descripcion'].'</td>
                            </tr>';
            };
            $html=$html.'</table><br>';
...
$html = $htmlstyle.'<h4>Metodología y Resultados Esperados</h4>    
    <table border="0">
    <tr>
        <th scope="col">Metodología y Resultados Esperados</th>
    </tr>
    <tr>
        <td>'.$PRY_MetodologiaResultadoEsperado.'</td>
    </tr>
    </table><br>';
...
$INF_Path='d:/DocumentosSistema/dialogosocial/'.$PRY_Carpeta.'/informes/informeinicial/';
if (!is_dir($INF_Path)) {
