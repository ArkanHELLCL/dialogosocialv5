			...
			<form role="form" action="<%=action%>" method="POST" name="frm11acepta" id="frm11acepta" class="needs-validation">
				<div class="row align-items-center">
					...
					<div class="col-sm-12 col-md-6 col-lg-6">
						<blockquote>
							<p>
								Después de haber revisado y corregido la información en los pasos anteriores, es necesario aceptar el informe "Avances", con el fin informar al ejecutor este hecho y asi poder continuar con el ingreso de la información del siguiente informe.
							</p>
							<p>
...
<div class="modal fade in" id="modalRechaza" tabindex="-1" role="dialog" aria-labelledby="modalRechazaLabel" aria-hidden="true">
	<div class="modal-dialog cascading-modal narrower modal-lg" role="document">  		
    	<div class="modal-content">		
			...
			<form role="form" action="/mod-11-h2-rechaza" method="POST" name="frmrechazainicio" id="frmrechazainicio" class="needs-validation">
					...
					if(data.state=="200"){						
						...
						if(VPR_EstadoAprobadoTotal<VPR_Total){
							swalWithBootstrapButtons.fire({
								...
								text:'Antes de aceptar el informe AVANCES, debes tener APROBADOS todos los documentos solicitados en el ítem "Informe"'
							});	
						}else{
							swalWithBootstrapButtons.fire({
								...
								$.ajax({
									...
									success: function(data) {						
										if(data.state=="200"){
											...
											wrk_informes('/prt-informeavancesmesas','informeavancesmesas.pdf',<%=PRY_Id%>,'<%=PRY_Identificador%>','/mnu-11',<%=session("ds5_usrid")%>,'<%=session("ds5_usrtoken")%>');
