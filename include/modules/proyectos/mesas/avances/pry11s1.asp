...
<form role="form" action="<%=action%>" method="POST" name="frm11s1" id="frm11s1" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosava11" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosava11" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm11s1" name="btn_frm11s1"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableAva11;		
		loadTableDocumentos();
        $('#tbl-documentosava11').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosava11")){				
				$('#tbl-documentosava11').dataTable().fnClearTable();
    			$('#tbl-documentosava11').dataTable().fnDestroy();
			}	
			
			documentosTableAva11 = $('#tbl-documentosava11').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){										
										documentosTableAva11.ajax.reload();
										Toast.fire({
										icon: 'success',
