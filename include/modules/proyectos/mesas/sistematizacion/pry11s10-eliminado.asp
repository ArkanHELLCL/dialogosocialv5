...
<%
	...
	if(mode="mod" or mode="vis") then		
		...
		if not rs.eof then	
			...
			PRY_SeminarioResultados=rs("PRY_SeminarioResultados")			
		else
	...
	<h5>Seminario de Resultados</h5>
	<form role="form" action="<%=action%>" method="POST" name="frm11s10" id="frm11s10" class="needs-validation">
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">								
						<i class="fas fa-comment prefix"></i>
							<textarea id="PRY_SeminarioResultados" name="PRY_SeminarioResultados" class="md-textarea form-control" <%=disabled%> rows="15"><%=PRY_SeminarioResultados%></textarea>
						<span class="select-bar"></span><%
						clase=""
						if(PRY_SeminarioResultados<>"") then
							clase="active"
						end if%>
						<label for="" class="<%=clase%>">Seminario de Resultados</label>									
					</div>
				</div>
			</div>
		</div>
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm11s10" name="btn_frm11s10"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm11s10").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm11s10");						
			if($("#frm11s10").valid()){
                $.ajax({
                    type: 'POST',			
                    url: $("#frm11s10").attr("action"),
                    data: $("#frm11s10").serialize(),
                    success: function(data) {								
                        var param=data.split(bb)
                        if(param[0]=="200"){
                            Toast.fire({
                                icon: 'success',
                                title: 'Seminario de Resultados grabados correctamente'
                            });
