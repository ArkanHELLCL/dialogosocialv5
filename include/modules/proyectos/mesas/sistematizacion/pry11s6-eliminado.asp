...
<%
	...
	<form role="form" action="<%=action%>" method="POST" name="frm11s6" id="frm11s6" class="needs-validation">
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">								
						<i class="fas fa-comment prefix"></i>
							<textarea id="PRY_Desafios" name="PRY_Desafios" class="md-textarea form-control" <%=disabled%> rows="15"><%=PRY_Desafios%></textarea>
						<span class="select-bar"></span><%
						clase=""
						if(PRY_Desafios<>"") then
							clase="active"
						end if%>
						<label for="" class="<%=clase%>">Desafíos y Continuidad</label>									
					</div>
				</div>
			</div>
		</div>
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm11s6" name="btn_frm11s6"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm11s6").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm11s6");						
			if($("#frm11s6").valid()){
				
					$.ajax({
						type: 'POST',			
						url: $("#frm11s6").attr("action"),
						data: $("#frm11s6").serialize(),
						success: function(data) {								
							var param=data.split(bb)
							if(param[0]=="200"){
								Toast.fire({
								  icon: 'success',
								  title: 'Dasafíos y Continuidad grabados correctamente'
								});
