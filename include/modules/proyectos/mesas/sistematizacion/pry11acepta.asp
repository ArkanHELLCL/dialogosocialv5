...
<%
	...
	if(mode="mod") then%>
		...
		<div style="padding-top:50px;"></div>
			<form role="form" action="<%=action%>" method="POST" name="frm11acepta" id="frm11acepta" class="needs-validation">
				<div class="row align-items-center">
					<div class="col-sm-12 col-md-6 col-lg-6" style="text-align:center;height:100%">
						<button type="button" class="btn btn-success btn-lg" id="btn_frm11acepta_1" name="btn_frm11acepta_1" value="enviar"><i class="fas fa-thumbs-up"></i> ACEPTAR</button>
						<button type="button" class="btn btn-danger btn-lg" id="btn_frm11acepta_2" name="btn_frm11acepta_2" value="enviar" data-toggle="modal" data-target="#modalRechaza"><i class="fas fa-thumbs-down"></i> RECHAZAR</button>
					</div>				
...
<div class="modal fade in" id="modalRechaza" tabindex="-1" role="dialog" aria-labelledby="modalRechazaLabel" aria-hidden="true">
	<div class="modal-dialog cascading-modal narrower modal-lg" role="document">  		
    	<div class="modal-content">		
			...
			<form role="form" action="/mod-11-h3-rechaza" method="POST" name="frmrechazainicio" id="frmrechazainicio" class="needs-validation">
...
<script>
	$(document).ready(function() {	
		...
		$("#btn_frm11acepta_1").click(function(){
			$.ajax({
				type: 'POST',			
					...
					if(data.state=="200"){						
						...
						if(VPR_EstadoAprobadoTotal<VPR_Total){
							swalWithBootstrapButtons.fire({
								...
								text:'Antes de aceptar el informe FINAL, debes tener APROBADOS todos los documentos solicitados en el ítem "Informe"'
							});	
						}else{
							$.ajax({
								type: 'POST',			
									...
									if(data.state=="200"){						
										...
										if(VER_EstadoAprobado<VER_Total){
											swalWithBootstrapButtons.fire({
												...
												text:'Antes de cerrar el informe FINAL, debes tener APROBADOS todos los documentos solicitados en el ítem "Verificadores"'
											});
										}else{
											swalWithBootstrapButtons.fire({
											...
											if (result.value) {			  					
												$.ajax({
													type: 'POST',			
													url: $("#frm11acepta").attr("action"),
													data: $("#frm11acepta").serialize(),
													dataType: "json",
													success: function(data) {						
														if(data.state=="200"){
															$("#frm11acepta")[0].reset();
															...
															wrk_informes('/prt-informefinalmesas','informefinalmesas.pdf',<%=PRY_Id%>,'<%=PRY_Identificador%>','/mnu-11',<%=session("ds5_usrid")%>,'<%=session("ds5_usrtoken")%>');
