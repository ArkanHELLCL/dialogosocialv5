				...
				<div class="md-form">
					<div class="error-message">								
						<i class="fas fa-comment prefix"></i>
							<textarea id="PRY_Sugerencias" name="PRY_Sugerencias" class="md-textarea form-control" <%=disabled%> rows="15"><%=PRY_Sugerencias%></textarea>
						<span class="select-bar"></span><%
						clase=""
						if(PRY_Sugerencias<>"") then
							clase="active"
						end if%>
						<label for="" class="<%=clase%>">Sugerencias de seguimiento y monitoreo</label>									
					</div>
				</div>
			</div>
		</div>
					...
					$.ajax({
						...
						success: function(data) {								
							...
							if(param[0]=="200"){
								Toast.fire({
								  icon: 'success',
								  title: 'Sugerencias grabados correctamente'
								});
