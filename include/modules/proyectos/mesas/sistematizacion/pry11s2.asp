...
<form role="form" action="<%=action%>" method="POST" name="frm11s2" id="frm11s2" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentossis11" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentossis11" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm11s2" name="btn_frm11s2"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableSis11;		
		loadTableDocumentos();
        $('#tbl-documentossis11').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentossis11")){				
				$('#tbl-documentossis11').dataTable().fnClearTable();
    			$('#tbl-documentossis11').dataTable().fnDestroy();
			}	
			
			documentosTableSis11 = $('#tbl-documentossis11').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){
										documentosTableSis11.ajax.reload();
										Toast.fire({
										icon: 'success',
