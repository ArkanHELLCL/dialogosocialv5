...
<%
	...
	if(mode="mod" or mode="vis") then		
		...
		if not rs.eof then	
			...
			PRY_TematicaGeneral=rs("PRY_TematicaGeneral")			
		else
	...
	<h5>Temática General y Transversal</h5>
	<form role="form" action="<%=action%>" method="POST" name="frm11s8" id="frm11s8" class="needs-validation">
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">								
						<i class="fas fa-comment prefix"></i>
							<textarea id="PRY_TematicaGeneral" name="PRY_TematicaGeneral" class="md-textarea form-control" <%=disabled%> rows="15"><%=PRY_TematicaGeneral%></textarea>
						<span class="select-bar"></span><%
						clase=""
						if(PRY_TematicaGeneral<>"") then
							clase="active"
						end if%>
						<label for="" class="<%=clase%>">Temática General y Transversal</label>									
					</div>
				</div>
			</div>
		</div>
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm11s8" name="btn_frm11s8"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm11s8").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm11s8");						
			if($("#frm11s8").valid()){
                $.ajax({
                    type: 'POST',			
                    url: $("#frm11s8").attr("action"),
                    data: $("#frm11s8").serialize(),
                    success: function(data) {								
                        var param=data.split(bb)
                        if(param[0]=="200"){
                            Toast.fire({
                                icon: 'success',
                                title: 'Temática General y Transversal grabadas correctamente'
                            });
