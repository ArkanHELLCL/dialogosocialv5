...
<%
	...
	'response.end
%>
<form role="form" action="" method="POST" name="frm11s9_1" id="frm11s9_1" class="needs-validation">
	...
	<div class="row">
		...
		<div class="col-sm-12 col-md-9 col-lg-9">
			<div class="md-form input-with-post-icon">
				<div class="error-message">
					<div class="select">
						<select name="REL_Id" id="REL_Id" class="validate select-text form-control" <%=disabled%> data-msg="Por favor, ingrese un relator">
	...
	<div class="row align-items-center">
		...
		<div class="col-sm-12 col-md-1 col-lg-1" style="text-align:left;">
			<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm11s9_1" name="btn_frm11s9_1" style="float:right;"><i class="fas fa-plus"></i></button>
		</div>
	</div><%
	end if%>
...
<form role="form" action="<%=action%>" method="POST" name="frm11s9" id="frm11s9" class="needs-validation">
	<div class="row">		
		<div class="footer"><%
			if mode="mod" or mode="add" then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm11s9" name="btn_frm11s9"><%=txtBoton%></button><%
			else%>
	...
	<input type="hidden" id="Step" name="Step" value="9" />	
	<input type="hidden" id="PRY_Hito" name="PRY_Hito" value="3" />	
</form>
	...
	$(document).ready(function() {				
		...
		function oblgrela(){
			$.ajax({
				...
				success: function (data) {							
					if(data.state==200){
						//console.log(parseInt(data.TIM_RelatorObligatorio));
						if(parseInt(data.TIM_RelatorObligatorio)==1){
		...
		$("#btn_frm11s9_1").click(function(){
			formValidate("#frm11s9_1")
			if($("#frm11s9_1").valid()){
				$.ajax({
					type: 'POST',			
					url: "/grabar-plan-de-trabajo",
					data: $("#frm11s9_1").serialize(),
					dataType:"json",
					success: function(data) {	
						if(data.state==200){						
							plantrabajoMESA.ajax.reload();	
		...
		$("#btn_frm11s9").click(function(){
			if(plantrabajoMESA.data().count()>0){
				var bb = String.fromCharCode(92) + String.fromCharCode(92);
				$.ajax({
					type: 'POST',					
					url: $("#frm11s9").attr("action"),
					data: $("#frm11s9").serialize(),					
					success: function(data) {						
						param=data.split(bb);
						if(param[0]=="200"){
							Toast.fire({
