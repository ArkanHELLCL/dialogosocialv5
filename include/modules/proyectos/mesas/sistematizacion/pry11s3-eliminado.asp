		...
		<div class="container-nav" style="margin-right: 15px;margin-left: 15px;">
			<div class="header">				
				<div class="content-nav">
					<a id="sis-sin1-tab" href="#sis-sintab1" class="active tab"><i class="fas fa-users"></i> Representantes Sindicato 						
					</a>
					<a id="sis-emp2-tab" href="#sis-emptab2" class="tab"><i class="fas fa-industry"></i> Representantes Empresa 						
					</a><%
					if PRY_TipoMesa=2 then		'Tripartita%>
						<a id="sis-gob3-tab" href="#sis-gobtab3" class="tab"><i class="fas fa-university"></i> Representantes Gobierno 							
						</a><%
					end if%>
			...
			<div class="tab-content">
				...
				<div id="sis-sintab1"><%
					...
					do while not rz.eof
						...
						<table id="tbl-sisrepsind_<%=x%>" class="ts table table-striped table-bordered dataTable table-sm tbl-sisrepsind" data-id="sisrepsind" data-page="true" data-selected="true" data-keys="1"> 
							<thead> 
								<tr> 
							...
							<tbody><%
								...
								loop
								dataSindicales=dataSindicales & "]"
								dataSISSindicalesArray=dataSISSindicalesArray & dataSindicales & ","
								rs.close%>                	
							</tbody>
						</table><%
						rz.movenext
					loop
					MaxSin = x
					dataSISSindicalesArray = "[" & dataSISSindicalesArray & "]"
					if MaxSin = 0 then
						MaxSin=1%>
						<h5>Sindicato sin representantes</h5>
						<h6 style="padding-bottom:20px;">Representantes del Sindicato</h6>
						<table id="tbl-sisrepsind_1" class="ts table table-striped table-bordered dataTable table-sm tbl-sisrepsind" data-id="sisrepsind" data-page="true" data-selected="true" data-keys="1"> 
							<thead> 
								<tr> 
									<th style="width:10px;">#</th>
									<th>Nombre</th>
				...
				<!--sis-emptab2-->
				<div id="sis-emptab2">	<%'Organizaciones Empresariales
					x=0
					...
					do while not rz.eof
						...
						<table id="tbl-sisrepemp_<%=x%>" class="ts table table-striped table-bordered dataTable table-sm tbl-sisrepemp" data-id="sisrepemp" data-page="true" data-selected="true" data-keys="1"> 
							<thead> 
								<tr> 
									<th style="width:10px;">#</th>
									<th>Nombre</th>
							...
							<tbody><%
								...
								loop
								dataEmpresariales=dataEmpresariales & "]"
								dataSISEmpresarialesArray=dataSISEmpresarialesArray & dataEmpresariales & ","
								rs.close%>                	
							</tbody>
						</table><%
						rz.movenext
					loop
					MaxEmp = x
					dataSISEmpresarialesArray = "[" & dataSISEmpresarialesArray & "]"
					if MaxEmp = 0 then
						MaxEmp=1%>
						<h5>Empresa sin representantes</h5>
						<h6 style="padding-bottom:20px;">Representantes del Sindicato</h6>
						<table id="tbl-sisrepemp_1" class="ts table table-striped table-bordered dataTable table-sm tbl-sisrepemp" data-id="sisrepemp" data-page="true" data-selected="true" data-keys="1"> 
							<thead> 
								<tr> 
									<th style="width:10px;">#</th>
									<th>Nombre</th>
				...
				if PRY_TipoMesa=2 then		'Tripartita%>
					<div id="sis-gobtab3"><%'Organizaciones de gobierno
						x=0
						...
						do while not rz.eof
							...
							<table id="tbl-sisrepgob_<%=x%>" class="ts table table-striped table-bordered dataTable table-sm tbl-sisrepgob" data-id="sisrepgob" data-page="true" data-selected="true" data-keys="1"> 
								<thead> 
									<tr> 
										<th style="width:10px;">#</th>
										<th>Nombre</th>
								...
								<tbody><%
									...
									loop
									dataGobierno=dataGobierno & "]"	
									dataSISGobiernoArray=dataSISGobiernoArray & dataGobierno & ","
									rs.close%>                	
								</tbody>
							</table><%
							rz.movenext
						loop
						MaxGob = x
						dataSISGobiernoArray = "[" & dataSISGobiernoArray & "]"
						if MaxGob = 0 then
							MaxGob=1%>
							<h5>Servicio sin representantes</h5>
							<h6 style="padding-bottom:20px;">Representantes del Sindicato</h6>
							<table id="tbl-sisrepgob_1" class="ts table table-striped table-bordered dataTable table-sm tbl-sisrepgob" data-id="sisrepgob" data-page="true" data-selected="true" data-keys="1"> 
								<thead> 
									<tr> 
										<th style="width:10px;">#</th>
										<th>Nombre</th>
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm11s3_1").click(function(){
			
		})				
		
		var dataSISSindicalesArray = [];
		var tableSinSIS;
		dataSISSindicalesArray = <%=dataSISSindicalesArray%>		
		
		for(i=1;i<=<%=MaxSin%>;i++){
			tableSinSIS = '#tbl-sisrepsind_' + i
			loadtableSinSISdicales(dataSISSindicalesArray[i-1],tableSinSIS);
        	$(tableSinSIS).css('width','100%')
		}
		
		function loadtableSinSISdicales(data,table) {			
			$(table).DataTable({				
				lengthMenu: [ 5,10,20 ],
				data:data,
				columnDefs: [ {
		...
		var dataSISEmpresarialesArray = [];
		var tableEmpSIS;
		dataSISEmpresarialesArray = <%=dataSISEmpresarialesArray%>		
		
		for(i=1;i<=<%=MaxEmp%>;i++){
			tableEmpSIS = '#tbl-sisrepemp_' + i
			loadtableEmpSISresariales(dataSISEmpresarialesArray[i-1],tableEmpSIS);
        	$(tableEmpSIS).css('width','100%')
		}
		
		function loadtableEmpSISresariales(data,table) {				
			$(table).DataTable({				
				lengthMenu: [ 5,10,20 ],
				data:data,
				columnDefs: [ {
		...
		$("#btn_frm11s3_2").click(function(){
			
		})		
		
		var TipoMesa = <%=PRY_TipoMesa%>;		
		if(TipoMesa==2){
			var dataSISGobiernoArray = [];
			var tableGobSIS;
			dataSISGobiernoArray = <%=dataSISGobiernoArray%>		
			for(i=1;i<=<%=MaxGob%>;i++){
				tableGobSIS = '#tbl-sisrepgob_' + i
				loadtableGobSISierno(dataSISGobiernoArray[i-1],tableGobSIS);
				$(tableGobSIS).css('width','100%')
			}

			function loadtableGobSISierno(data,table) {		
				$(table).DataTable({				
					lengthMenu: [ 5,10,20 ],
					data:data,
					columnDefs: [ {
		...
		$("#btn_frm11s3").click(function(){
			...
			if($("#frm11s3").valid()){
				...
				if($("#PRY_TipoMesa").val()==2){
					if(dataSISSindicalesArray.length>0 && dataSISEmpresarialesArray.length>0 && dataSISGobiernoArray.length>0){
						valido=true;
					}
				}else{
					if(dataSISSindicalesArray.length>0 && dataSISEmpresarialesArray.length>0){
						valido=true;
					}
				}				
				if(valido){
