...
<%
	...
	<h5>Diagnóstico Social Laboral</h5>
	<form role="form" action="<%=action%>" method="POST" name="frm11s4" id="frm11s4" class="needs-validation">			
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">								
						...
						<label for="" class="<%=clase%>">Diagnóstico Social Laboral</label>									
					</div>
				</div>
			</div>
		</div>			
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm11s4" name="btn_frm11s4"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm11s4").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm11s4");			
						
			if($("#frm11s4").valid()){
				
					$.ajax({
						type: 'POST',			
						url: $("#frm11s4").attr("action"),
						data: $("#frm11s4").serialize(),
