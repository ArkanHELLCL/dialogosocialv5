...
<div class="modal fade in" id="modalRechaza" tabindex="-1" role="dialog" aria-labelledby="modalRechazaLabel" aria-hidden="true">
	<div class="modal-dialog cascading-modal narrower modal-lg" role="document">  		
    	<div class="modal-content">		
			...
			<form role="form" action="/mod-10-h2-rechaza" method="POST" name="frmrechazaparcial" id="frmrechazaparcial" class="needs-validation">
				<div class="modal-body">
		  		...
		  		<div class="modal-footer">
					...
					<button type="button" class="btn btn-danger btn-md waves-effect" id="btn_rechazaparcial" name="btn_rechazaparcial"><i class="fas fa-times"></i> Rechazar</button>
				</div>
				<input type="hidden" id="PRY_Id" name="PRY_Id" value="<%=PRY_Id%>">
				<input type="hidden" id="PRY_Identificador" name="PRY_Identificador" value="<%=PRY_Identificador%>">
			</form>
...
<script>
	$(document).ready(function() {	
								...
								$.ajax({
									...
									success: function(data) {						
										if(data.state=="200"){
											...
											wrk_informes('/prt-informeparcialescuela','informeparcial.pdf',<%=PRY_Id%>,'<%=PRY_Identificador%>','/mnu-10',<%=session("ds5_usrid")%>,'<%=session("ds5_usrtoken")%>');
		...
		$("#btn_rechazaparcial").click(function(){		
			formValidate("#frmrechazaparcial");
			if($("#frmrechazaparcial").valid()){	
				...
				swalWithBootstrapButtons.fire({
				  ...
				  if (result.value) {			  	
					$.ajax({
						type: 'POST',			
						url: $("#frmrechazaparcial").attr("action"),
						data: $("#frmrechazaparcial").serialize(),
						dataType: "json",
						success: function(data) {												
							if(data.state=="200"){
								$("#frmrechazaparcial")[0].reset();								
								Toast.fire({
								  icon: 'success',
