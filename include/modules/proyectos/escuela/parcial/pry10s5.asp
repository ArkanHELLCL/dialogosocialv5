...
<%
	...
	if(PRY_InformeParcialEstado="" or IsNULL(PRY_InformeParcialEstado)) then
		PRY_InformeParcialEstado=0
	end if
	if(PRY_InformeParcialAceptado="" or IsNULL(PRY_InformeParcialAceptado)) then
		PRY_InformeParcialAceptado=0
	end if
	'if(PRY_InformeParcialEstado=0) or session("ds5_usrperfil")=1 then
	if((PRY_InformeParcialEstado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=3)) or ((PRY_InformeParcialEstado=1 and not PRY_InformeParcialAceptado) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2)) then
		mode="mod"
	else
		mode="vis"
	end if
...
<form role="form" action="<%=action%>" method="POST" name="frm10s5" id="frm10s5" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentospar10" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentospar10" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm10s5" name="btn_frm10s5"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTablePar10;		
		loadTableDocumentos();
        $('#tbl-documentospar10').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentospar10")){				
				$('#tbl-documentospar10').dataTable().fnClearTable();
    			$('#tbl-documentospar10').dataTable().fnDestroy();
			}	
			
			documentosTablePar10 = $('#tbl-documentospar10').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {																
									if(data.state==200){											
										documentosTablePar10.ajax.reload();
										Toast.fire({
										icon: 'success',
