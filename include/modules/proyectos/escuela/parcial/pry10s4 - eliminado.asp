...
<%
	...
	<h5>Seguimiento Ejecución</h5>
	<h6>Ingrese una breve reseña del estado de avance de la ejecución, indicando facilitadores y obstaculizadores del proceso de formación. Hacer especial énfasis en identificar situaciones de riesgo y los respectivos mecanismos de mitigación.</h6>
	<form role="form" action="<%=action%>" method="POST" name="frm10s1" id="frm10s4" class="needs-validation">
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">					
						<i class="fas fa-comment prefix"></i>
						<textarea id="PRY_Facilitadores" name="PRY_Facilitadores" class="md-textarea form-control" rows="3" <%=disabled%>><%=PRY_Facilitadores%></textarea>
						<span class="select-bar"></span><%
						if(PRY_Facilitadores<>"") then
							clase="active"
						else
							clase=""
						end if%>
						<label for="PRY_Facilitadores" class="<%=clase%>">Facilitadores</label>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">					
						<i class="fas fa-comment prefix"></i>
						<textarea id="PRY_Obstaculizadores" name="PRY_Obstaculizadores" class="md-textarea form-control" rows="3" <%=disabled%>><%=PRY_Obstaculizadores%></textarea>
						<span class="select-bar"></span><%
						if(PRY_Obstaculizadores<>"") then
							clase="active"
						else
							clase=""
						end if%>
						<label for="PRY_Obstaculizadores" class="<%=clase%>">Obstaculizadores</label>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">					
						<i class="fas fa-comment prefix"></i>
						<textarea id="PRY_MecMitigacion" name="PRY_MecMitigacion" class="md-textarea form-control" rows="3" <%=disabled%>><%=PRY_MecMitigacion%></textarea>
						<span class="select-bar"></span><%
						if(PRY_MecMitigacion<>"") then
							clase="active"
						else
							clase=""
						end if%>
						<label for="PRY_MecMitigacion" class="<%=clase%>">Mecanismos de mitigación</label>
					</div>
				</div>
			</div>
		</div>
				...
				$.ajax({
					...
					success: function(data) {						
						...
						if(param[0]=="200"){
							Toast.fire({
							  icon: 'success',
							  title: 'Seguimiento Ejecución grabado correctamente'
							});
							var modo = <%=modo%>;
							var PRY_Id = <%=PRY_Id%>;
							if(modo==1){
