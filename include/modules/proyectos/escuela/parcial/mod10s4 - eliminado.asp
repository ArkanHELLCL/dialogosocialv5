...
<%
	...
	PRY_Facilitadores				 = LimpiarUrl(request("PRY_Facilitadores"))
	PRY_Obstaculizadores			 = LimpiarUrl(request("PRY_Obstaculizadores"))
	PRY_MecMitigacion				 = LimpiarUrl(request("PRY_MecMitigacion"))
	...
	if PRY_Step=Step and PRY_InformeParcialEstado=0 then
		PRY_Step = PRY_Step + 1	'Siguiente paso
	end if		
	
	datos =  PRY_Id & ",'" & PRY_Identificador & "'," & PRY_Step & ",'" & PRY_Facilitadores & "','" & PRY_Obstaculizadores & "','" & PRY_MecMitigacion & "'," & session("ds5_usrid") & ",'" & session("ds5_usrtoken") & "'"


	sql="exec spProyectoInformeDesarrollo_Modificar " & datos 	
