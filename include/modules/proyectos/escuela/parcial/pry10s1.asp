	...
	<div class="row px-4" style="padding-top:30px;">		
		<table id="tbl-obsevaciones" class="ts table table-striped table-bordered dataTable table-sm" data-id="obsevaciones" data-page="true" data-selected="true" data-keys="1" width="100%"> 
			...
			do while not rsx.eof										
				if rsx("EST_Estado")=6 and rsx("EST_InfoEstadoAcademico")<>99 then 'rsx("EST_InfoObservaciones")<>"" then%>
					<tr>
			...
			<div class="md-form input-with-post-icon">
				<div class="error-message">								
					<i class="fas fa-calendar input-prefix"></i>
						<input type="text" id="ALU_FechaDesercion" name="ALU_FechaDesercion" class="form-control calendario hasDatepicker" readonly="" value="<%=mid(FechaPrimeraDesercion,1,10)%>">
					<span class="select-bar"></span>
					<label for="ALU_FechaDesercion" class="active">Fecha Primera Deserción</label>									
				</div>
			</div>
		</div>				
	</div>
				...
				$.ajax({
					...
					success: function(data) {						
						...
						if(param[0]=="200"){
							Toast.fire({
							  icon: 'success',
							  title: 'Causas de Deserciones grabadas correctamente'
							});
							var modo = <%=modo%>;
							var PRY_Id = <%=PRY_Id%>;
							if(modo==1){
