...
<%
	...
	'response.end%>
	<h5>Módulos adicionales</h5><%
	if(mode="mod") then%>
		<h6>Nombre del Módulo</h6>
		<form role="form" action="/agregar-tematica" method="POST" name="frm10s5_1" id="frm10s5_1" class="needs-validation">			
			<div class="row"> 
				<div class="col-xs-12 col-sm-12 col-md-11 col-lg-11">
					<div class="md-form input-with-post-icon">
						<div class="error-message">								
							...
							<label for="TPR_Nombre" class="<%=lblClass%>">Módulo adicional</label>									
						</div>
					</div>
				</div>			
				<div class="col align-self-end">
					<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm10s5_1" name="btn_frm10s5_1" style="float: right;"><i class="fas fa-plus"></i></button>
				</div>
			</div>
	...
	<div class="row" style="padding-top:20px">		
		<div class="col-12">
			<span class="text-muted" style="float:left;Padding-bottom:10px">Módulos adicionales incorporados</span>
...
<script>
	$(document).ready(function() {	
		...
		$("#btn_frm10s5_1").click(function(){
			formValidate("#frm10s5_1")
			if($("#frm10s5_1").valid()){
				$.ajax({
					type: 'POST',			
					url: $("#frm10s5_1").attr("action"),
					data: $("#frm10s5_1").serialize(),
		...
		$("#pry-content").on("click",".delmodadd",function(e){
			e.preventDefault();
			e.stopPropagation();
			var TPR_Id = $(this).find("i").data("tpr");			
						...
						success: function(data) {					
							...
							if(param[0]=="200"){
								...
								swalWithBootstrapButtons.fire({
									icon:'error',								
									title: 'ERROR!',
									text:'No se pudo eliminar el módulo adicional. Revisa si es que ya tiene planificación ingresada.'
								});
							}
						},
						error: function(XMLHttpRequest, textStatus, errorThrown){
