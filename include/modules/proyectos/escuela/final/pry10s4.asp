...
<form role="form" action="<%=action%>" method="POST" name="frm10s4" id="frm10s4" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosfin10" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosfin10" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm10s4" name="btn_frm10s4"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableFin10;		
		loadTableDocumentos();
        $('#tbl-documentosfin10').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosfin10")){				
				$('#tbl-documentosfin10').dataTable().fnClearTable();
    			$('#tbl-documentosfin10').dataTable().fnDestroy();
			}	
			
			documentosTableFin10 = $('#tbl-documentosfin10').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {																
									if(data.state==200){
										documentosTableFin10.ajax.reload();
										Toast.fire({
										icon: 'success',
