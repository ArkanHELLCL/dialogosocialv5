...
<%		
	...
	if not rs.eof then
		...
		path="D:\DocumentosSistema\dialogosocial\" & carpeta & "\curriculums\" & carpeta_informe
	else
	...
	response.write("200\\")%>	
	<table id="tbl-archivoencargados" class="table table-striped table-bordered table-sm" data-id="archivoencargados" data-page="false" data-selected="false" data-keys="0" style="margin-top:20px;" width="100%" data-path="<%=path%>"> 
	<thead> 
		<tr> 
			<th>Corr</th> 
			<th>Nombre</th>
	...
	<tbody><%
		...
		For Each fichero IN directorio.Files
			...
			<tr> 
				...
				<th>
					<i class="fas fa-cloud-download-alt text-primary arcenc" data-file="<%=fichero.Name%>" data-pry="<%=PRY_Id%>" data-identificador="<%=PRY_Identificador%>" data-hito="<%=PRY_Hito%>" style="cursor:pointer;"></i>					
				</th>
			</tr><%
		Next%>
	</tbody>                 
