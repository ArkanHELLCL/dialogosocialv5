...
<form role="form" action="<%=action%>" method="POST" name="frm10s6" id="frm10s6" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosini10" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosini10" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm10s6" name="btn_frm10s6"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		loadTableDocumentos();
        $('#tbl-documentosini10').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosini10")){				
				$('#tbl-documentosini10').dataTable().fnClearTable();
    			$('#tbl-documentosini10').dataTable().fnDestroy();
			}	
			
			documentosTableIni10 = $('#tbl-documentosini10').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
