...
<%
	...
	if(PRY_CreacionProyectoEstado=0) and (PRY_Hito>0 or PRY_Hito="") then
		...
		if((PRY_InformeInicioEstado=0) and (PRY_Hito>1 or PRY_Hito="")) or (PRY_InformeInicioEstado=1 and PRY_InformeInicioAceptado=0 and PRY_Hito="") then
			...
			if((PRY_InformeParcialEstado=0) and (PRY_Hito>2 or PRY_Hito="")) or (PRY_InformeParcialEstado=1 and PRY_InformeParcialAceptado=0 and PRY_Hito="") then
				PRY_Hito=2
			else
				if((PRY_InformeFinalEstado=0) and (PRY_Hito>3 or PRY_Hito="")) or (PRY_InformeFinalEstado=1 and PRY_InformeFinalAceptado=0 and PRY_Hito="") then
					PRY_Hito=3
				else
	...
	if PRY_Estado=1 then 'Modificar
		if PRY_Hito=0 and PRY_CreacionProyectoEstado=1 then	'Hito Creacion
			...
			if PRY_Hito=1 and PRY_InformeInicioEstado=1 and CRT_Step>6 then	'Hito Inicio
				...
				if PRY_Hito=2 and PRY_InformeParcialEstado=1 and CRT_Step>5 then 'Hito Parcial
					if(PRY_InformeParcialEstado=1 and PRY_InformeParcialAceptado=0) then
						PRY_Hito=2
						CRT_Step=6
					else				
						PRY_Hito=3
					...
					if PRY_Hito=3 and PRY_InformeFinalEstado=1 and CRT_Step>=5 then 'Hito Final
	...
	do
		select case PRY_Hito
			...
			case 2	'Parcial
				menulen=5
				do
					select case PRY_InformeParcialEstado
						case 0	'Abierto
							if(session("ds5_usrperfil")=1 or session("ds5_usrperfil")=3) then
								mode="mod"
								modo=2
							...
							exit do
						case 1	'Cerrado		
							if(not PRY_InformeParcialAceptado and (session("ds5_usrperfil")=2 or session("ds5_usrperfil")=1)) and CRT_Step=menulen+1 then
								mode="mod"
								modo=2
								caso=6
							end if
	...
	yCRT_Step=CRT_Step
	if(CRT_Step=menulen and not LIN_AgregaTematica and PRY_Hito=0) then
		'yCRT_Step=CRT_Step + 1			
	end if
	if(PRY_Hito="") then
		PRY_Hito=0
	end if
	pryarc="pry-10-h" & PRY_Hito & "-s" & yCRT_Step
	if(PRY_Hito=0) then	'Creacion
		if(CRT_Step=menulen+1 and not LIN_AgregaTematica) then
			pryarc="pry-10-h0-fin"
		end if
	...
	if(PRY_Hito=1) then	'Inicio
		if(CRT_Step=menulen+1) then
			pryarc="pry-10-h1-fin"
		end if
		if(PRY_InformeInicioAceptado=0 and PRY_InformeInicioEstado=1 and (CRT_Step=menulen+1)) then
			pryarc="pry-10-h1-acepta"
		end if
	end if
	
	if(PRY_Hito=2) then	'Parcial
		if(CRT_Step=menulen+1) then
			pryarc="pry-10-h2-fin"
		end if
		if(PRY_InformeParcialAceptado=0 and PRY_InformeParcialEstado=1 and (CRT_Step=menulen+1)) then
			pryarc="pry-10-h2-acepta"
		end if
	end if
	
	if(PRY_Hito=3) then	'Final
		if(CRT_Step=menulen+1) then
			pryarc="pry-10-h3-fin"
		end if
		if(PRY_InformeFinalAceptado=0 and PRY_InformeFinalEstado=1 and (CRT_Step=menulen+1)) then
			pryarc="pry-10-h3-acepta"
		end if
	end if
	...
	function menu(xPRY_Hito,xPRY_Step,xCRT_Step,modo)		
		...
		if modo<>1 then
			...
			VER_SubidosPendientes = (ESC_EstadoSubidoTotal + PAT_EstadoSubidoTotal + ENP_EstadoSubidoTotal + PLC_EstadoSubidoTotal + PCO_EstadoSubidoTotal) - VER_RevisadosTotal		'Subidos no revisados
		...
		'"<i class='fas fa-thumbs-down'></i>Incumplimientos"
		if LFO_Calif=1 then
		...
		hitoscerrados=array(PRY_CreacionProyectoEstado,PRY_InformeInicioEstado,PRY_InformeParcialEstado,PRY_InformeFinalEstado)
		hitosaceptados=array(true,PRY_InformeInicioAceptado,PRY_InformeParcialAceptado,PRY_InformeFinalAceptado)
		hitos=array("Creación","Inicio","Desarrollo","Final")	
		hitoslen=3
		informesnombre=array("informecreacion.pdf","informeinicio.pdf","informeparcial.pdf","informefinal.pdf")
		informessubdir=array("informecreacion","informeinicio","informeparcial","informefinal")
		prtinformes=array("/prt-informecreacionescuela","/prt-informeinicioescuela","/prt-informeparcialescuela","/prt-informefinalescuela")
		...
		if LIN_AgregaTematica then			
			...
			menus(0,1)="Focalización de Beneficiarios"
			menus(0,2)="Fechas de Cierre"			
			'menus(0,3)="Enfoques Pedagógicos"
			'menus(0,3)="Estrategia de Convocatoria"
			menus(0,3)="Redes de Apoyo"
			menus(0,4)="Módulos Adicionales"
			'menus(0,6)="Planes"			
			menus(0,5)="Finalizar"
		else			
			menus(0,0)="Personalización"
		...
		menus(1,2)="Focalización de Beneficiarios"		
		menus(1,3)="Planificación"
		menus(1,4)="Actividades"									
		menus(1,5)="Informe Nro 1 " & texto
		...
		'Parcial
		menus(2,0)="Causas de Deserciones"
		menus(2,1)="Deserciones/Incorporaciones"
		menus(2,2)="Planificación por Ejecutar"
		'menus(2,3)="Seguimiento Ejecución"
		menus(2,3)="Informe de Asistencia"		
		menus(2,4)="Informe Nro 2 " & texto
		if(PRY_InformeParcialEstado=0) then			
			menus(2,5)="Finalizar"
		else
			if(PRY_InformeParcialAceptado=0) then
				menus(2,5)="Aceptar"
			end if
		end if		

		'Final		
		menus(3,0)="Cobertura del Programa"
		menus(3,1)="Causas de la Deserción"
		menus(3,2)="Informe de Asistencia"
		'menus(3,3)="Cumplimientos"
		'menus(3,4)="Evaluación del Programa"
		'menus(3,5)="Observaciones"		
		menus(3,3)="Informe Nro 3 " & texto		
		...
		if hitoscerrados(0)<>1 then
			...
			'for j=0 to menucierrelen	'Mostrando el menu de la cabecera mensajes del proyecto cuando el hito de creacion esta abierto
				salida = salida + "<li role='presentation' class='menus'><a role='tab' href='#' data-url='" & menucierrepag(4) & "' data-mode='" & modo & "' data-hito='" & xPRY_Hito & "' data-step='" & xCRT_Step & "'>" & menucierre(4) & "</a></li>"
			'next		
		end if
