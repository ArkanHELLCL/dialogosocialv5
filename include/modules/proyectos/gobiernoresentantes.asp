...
<%
	...
	dataRepGOB = "{""data"":["
	do While Not rs.EOF
		set rx = cnn.Execute("spProyectoServicio_Listar "  & PRY_Id & "," & rs("SER_Id"))
		on error resume next
		if cnn.Errors.Count > 0 then 
			ErrMsg = cnn.Errors(0).description
			response.write("Error spProyectoServicio_Listar")
			cnn.close 		
			response.end
		End If
		if not rx.eof then
		...
		If(Total>0) then
			detalle="<i class='fas fa-chevron-down text-secondary verrepgob' data-toggle='tooltip' title='Ver Representantes' data-pry='" & PRY_Id & "' data-ser='" & rs("SER_Id") & "'></i>"
		else
			detalle=""
		end if
		...
		if (PRY_InformeInicialEstado=0 and PRY_Estado=1) and ((session("ds5_usrperfil")=2) or (session("ds5_usrperfil")=1) or (session("ds5_usrperfil")=5)) then
			acciones = "<i class='fas fa-user-plus text-success addrepgob' data-toggle='tooltip' title='Agregar Representantes' data-pry='" & PRY_Id & "' data-ser='" & rs("SER_Id") & "'></i> " & detalle
		else
			acciones = detalle
		end if
		dataRepGOB = dataRepGOB & "[""" & rs("SER_Id") & """,""" & rs("SER_Nombre") & """,""" & rs("GOB_Nombre") & """,""" & Total & """,""" & acciones & """]" 
									
		rs.movenext
		if not rs.eof then
			dataRepGOB = dataRepGOB & ","
		end if
		
	loop
	dataRepGOB=dataRepGOB & "]}"	
	response.write(dataRepGOB)
%>