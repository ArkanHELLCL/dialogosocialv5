...
<%
	...
	<form role="form" action="<%=action%>" method="POST" name="frm12s4_evaluacion" id="frm12s4_evaluacion" class="needs-validation">
...
<script>
	...
	$(document).ready(function() {
		...
		$("#btn_frm12s4").click(function(){
			formValidate("#frm12s4_evaluacion")
			if($("#frm12s4_evaluacion").valid()){
				var bb = String.fromCharCode(92) + String.fromCharCode(92);
				$.ajax({
					type: 'POST',			
					url: $("#frm12s4_evaluacion").attr("action"),
					data: $("#frm12s4_evaluacion").serialize(),
