...
<%
	...
	<h5>Observaciones</h5>
	<h6>¿Que Módulos y Metodologías provocaron mayor interés en los estudiantes?</h6>
	<form role="form" action="<%=action%>" method="POST" name="frm12s6" id="frm12s6" class="needs-validation">
			...
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">					
						...
						<textarea id="PRY_ObsEjecucion" name="PRY_ObsEjecucion" class="md-textarea form-control" rows="3" <%=disabled%>><%=PRY_ObsEjecucion%></textarea>
						<span class="select-bar"></span><%
						if(PRY_ObsEjecucion<>"") then
							clase="active"
						else
							clase=""
						end if%>
						<label for="PRY_ObsEjecucion" class="<%=clase%>">Observación Ejecución</label>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">					
						<i class="fas fa-comment prefix"></i>
						<textarea id="PRY_ObsGeneral" name="PRY_ObsGeneral" class="md-textarea form-control" rows="3" <%=disabled%>><%=PRY_ObsGeneral%></textarea>
						<span class="select-bar"></span><%
						if(PRY_ObsGeneral<>"") then
							clase="active"
						else
							clase=""
						end if%>
						<label for="PRY_ObsGeneral" class="<%=clase%>">Observación General</label>
					</div>
				</div>
			</div>
		</div>		
		<div class="row">		
			<div class="footer"><%
				if mode="mod" then%>				
					<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm12s6" name="btn_frm12s6"><%=txtBoton%></button>
