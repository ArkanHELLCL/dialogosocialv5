...
<%
	...
	'if(PRY_InformeFinalEstado=0) or session("ds5_usrperfil")=1 then
	if((PRY_InformeFinalEstado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=3)) or ((PRY_InformeFinalEstado=1 and not PRY_InformeFinalAceptado) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2)) then
		mode="mod"
	else
		mode="vis"
	end if
...
<form role="form" action="<%=action%>" method="POST" name="frm12s4" id="frm12s4" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosfin12" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosfin12" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm12s4" name="btn_frm12s4"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableFin12;		
		loadTableDocumentos();
        $('#tbl-documentosfin12').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosfin12")){				
				$('#tbl-documentosfin12').dataTable().fnClearTable();
    			$('#tbl-documentosfin12').dataTable().fnDestroy();
			}	
			
			documentosTableFin12 = $('#tbl-documentosfin12').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {																
									if(data.state==200){
										documentosTableFin12.ajax.reload();
										Toast.fire({
										icon: 'success',
