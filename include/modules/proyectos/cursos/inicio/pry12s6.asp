...
<%
	...
	'if(PRY_InformeInicioEstado=0) or session("ds5_usrperfil")=1 then
	if((PRY_InformeInicioEstado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=3)) or ((PRY_InformeInicioEstado=1 and PRY_InformeInicioAceptado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2)) then
		mode="mod"
	else
		mode="vis"
	end if
	...
	'response.end
%>
<form role="form" action="<%=action%>" method="POST" name="frm12s6" id="frm12s6" class="needs-validation">
	<h5>Documentos</h5>	
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosini12" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosini12" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm12s6" name="btn_frm12s6"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		$("#btn_frm12s6").click(function(){
			formValidate("#frm12s6")
			if($("#frm12s6").valid()){				
				$.ajax({
					type: 'POST',			
						...
						if(data.state=="200"){							
							...
							if(VPR_EstadoSubidoTotal<VPR_Total){
								swalWithBootstrapButtons.fire({
									...
									text:'Antes de avanzar al cierre del hito INICIO, debes tener arriba todos los documentos aqui solicitados'
								});	
							}else{								
								var bb = String.fromCharCode(92) + String.fromCharCode(92);
								$.ajax({
		...
		var documentosTableIni10;		
		loadTableDocumentos();
        $('#tbl-documentosini12').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosini12")){				
				$('#tbl-documentosini12').dataTable().fnClearTable();
    			$('#tbl-documentosini12').dataTable().fnDestroy();
			}	
			
			documentosTableIni10 = $('#tbl-documentosini12').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){
										menu();
										documentosTableIni10.ajax.reload();
										Toast.fire({
										icon: 'success',
		...
		function menu(){
			var data   = {modo:<%=modo%>,PRY_Id:<%=PRY_Id%>,LIN_Id:<%=LIN_Id%>,CRT_Step:$("#Step").val(),Modulo:false};
			$.ajax( {
				type:'POST',					
				url: '/mnu-12',
				data: data,
