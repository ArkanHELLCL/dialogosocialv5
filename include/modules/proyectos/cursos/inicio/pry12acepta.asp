					...
					if(data.state=="200"){						
						...
						if(VPR_EstadoAprobadoTotal<VPR_Total){
							...
							swalWithBootstrapButtons.fire({
							  ...
							  text: '¿Estas seguro de querer aceptar el informe "INICIO" para dar comienzo al ingreso del informe Final?',
							  icon: 'success',
								...
								$.ajax({
									...
									success: function(data) {						
										if(data.state=="200"){
											...
											wrk_informes('/prt-informeinicialcursos','informeinicio.pdf',<%=PRY_Id%>,'<%=PRY_Identificador%>','/mnu-10',<%=session("ds5_usrid")%>,'<%=session("ds5_usrtoken")%>');
