...
<%
	...
	if mode="mod" then
		...
		action="/mod-12-h0-s3"
		columnsSIN=""
		if(LIN_AgregaSindicato and LIN_AgregaSindicatoVerificador) then
			...
			if(LIN_AgregaSindicato and not LIN_AgregaSindicatoVerificador) then
				columnsSIN="{data: ""SIN_Id""},{data: ""SIN_Nombre""},{data: ""ACE_Nombre""},{data: ""RUB_Nombre""},{data: ""PAT_Compromiso""},{className: 'delsin',orderable: false,data: ""Del""}"
			end if
		end if
		if(LIN_AgregaEmpresa and LIN_AgregaEmpresaVerificador) then
			...
			if(LIN_AgregaEmpresa and not LIN_AgregaEmpresaVerificador) then
				columnsEMP="{data: ""EMP_Id""},{data: ""EMP_Nombre""},{data: ""RUB_Nombre""},{data: ""PEM_Compromiso""},{className: 'delemp',orderable: false,data: ""Del""}"
			end if
		end if
		if(LIN_AgregaCivil and LIN_AgregaCivilVerificador) then
			columnsCIV="{data: ""CIV_Id""},{data: ""CIV_Nombre""},{data: ""RUB_Nombre""},{data: ""PCI_Compromiso""},{data: ""PCI_VerificadorCumplimiento""},{className: 'delciv',orderable: false,data: ""Del""}"
		else
			if(LIN_AgregaCivil and not LIN_AgregaCivilVerificador) then
				columnsCIV="{data: ""CIV_Id""},{data: ""CIV_Nombre""},{data: ""RUB_Nombre""},{data: ""PCI_Compromiso""},{className: 'delciv',orderable: false,data: ""Del""}"
			end if
		end if
		if(LIN_AgregaGobierno and LIN_AgregaGobiernoVerificador) then
			...
			if(LIN_AgregaGobierno and not LIN_AgregaGobiernoVerificador) then
				columnsGOB="{data: ""SER_Id""},{data: ""SER_Nombre""},{data: ""PGO_Compromiso""},{className: 'delpgo',orderable: false,data: ""Del""}"
			end if
		end if		
	end if
	if mode="vis" then
		...
		if(LIN_AgregaSindicato and LIN_AgregaSindicatoVerificador) then
			...
			if(LIN_AgregaSindicato and not LIN_AgregaSindicatoVerificador) then
				columnsSIN="{data: ""SIN_Id""},{data: ""SIN_Nombre""},{data: ""ACE_Nombre""},{data: ""RUB_Nombre""},{data: ""PAT_Compromiso""}"
			end if
		end if
		if(LIN_AgregaEmpresa and LIN_AgregaEmpresaVerificador) then
			...
			if(LIN_AgregaEmpresa and not LIN_AgregaEmpresaVerificador) then
				columnsEMP="{data: ""EMP_Id""},{data: ""EMP_Nombre""},{data: ""RUB_Nombre""},{data: ""PEM_Compromiso""}"
			end if
		end if
		if(LIN_AgregaCivil and LIN_AgregaCivilVerificador) then
			columnsCIV="{data: ""CIV_Id""},{data: ""CIV_Nombre""},{data: ""RUB_Nombre""},{data: ""PCI_Compromiso""},{data: ""PCI_VerificadorCumplimiento""}"
		else
			if(LIN_AgregaCivil and not LIN_AgregaCivilVerificador) then
				columnsCIV="{data: ""CIV_Id""},{data: ""CIV_Nombre""},{data: ""RUB_Nombre""},{data: ""PCI_Compromiso""}"
			end if
		end if
		if(LIN_AgregaGobierno and LIN_AgregaGobiernoVerificador) then
			...
			if(LIN_AgregaGobierno and not LIN_AgregaGobiernoVerificador) then
				columnsGOB="{data: ""SER_Id""},{data: ""SER_Nombre""},{data: ""PGO_Compromiso""}"
			end if
		end if		
	end if
	...
	<div class="row">
		...
		<div class="container-nav" style="margin-right: 15px;margin-left: 15px;">
			<div class="header">				
				<div class="content-nav"><%
					if(LIN_AgregaSindicato) then%>
						<a id="org1-12-tab" href="#sin12tab1" class="active tab"><i class="fas fa-users"></i> Organizaciones Sindicales 
							<span class="badge right blue badgesin">0</span>
						</a><%
					end if
					if(LIN_AgregaEmpresa) then%>					
						<a id="org2-12-tab" href="#emp12tab2" class="tab"><i class="fas fa-industry"></i> Organizaciones Empresariales 
							<span class="badge right blue badgeemp">0</span>
						</a><%
					end if
					if(LIN_AgregaCivil) then%>					
						<a id="org3-12-tab" href="#civ12tab3" class="tab"><i class="fas fa-building"></i> Organizaciones Civiles 
							<span class="badge right blue badgeciv">0</span>
						</a><%
					end if
					if(LIN_AgregaGobierno) then%>					
						<a id="org4-12-tab" href="#gob12tab3" class="tab"><i class="fas fa-university"></i> Organizaciones de Gobierno 
							<span class="badge right blue badgegob">0</span>
						</a><%
					end if
					if(not LIN_AgregaSindicato and not LIN_AgregaEmpresa and not LIN_AgregaCivil and not LIN_AgregaGobierno) then%>
						<a id="org5-12-tab" href="#not12tab3" class="tab"><i class="fas fa-times"></i> Sin Redes de Apoyo (Ver Mantenedor de Lineas) 							
						</a><%
					end if%>
			...
			<div class="tab-content"><%
				if(LIN_AgregaSindicato) then%>
					<div id="sin12tab1">	<%'Organizaciones Sindicales
						if(mode="mod") then%>
							<form role="form" action="/agregar-sindicato" method="POST" name="frm12s3_1" id="frm12s3_1" class="needs-validation">
								<div class="row">                                                                         
									...
									<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
										<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm12s3_1" name="btn_frm12s3_1"><i class="fas fa-plus"></i></button>	
									</div>						
								</div>	
				...
				if(LIN_AgregaEmpresa) then%>
					<div id="emp12tab2">	<%'Organizaciones Empresariales
						if(mode="mod") then%>
							<form role="form" action="/agregar-empresa" method="POST" name="frm12s3_2" id="frm12s3_2" class="needs-validation">				
								<div class="row">                                                                         
									<div class="col-sm-5 col-md-5 col-lg-5">
										<div class="md-form input-with-post-icon">
											<div class="error-message">
									...
									<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
										<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm12s3_2" name="btn_frm12s3_2"><i class="fas fa-plus"></i></button>	
									</div>						
								</div>	
								<input type="hidden" id="PRY_Id" value="<%=PRY_Id%>" name="PRY_Id">
							</form><%
						...
						<table id="tbl-orgsemp" class="ts table table-striped table-bordered dataTable table-sm" data-id="orgsemp" data-page="true" data-selected="true" data-keys="1"> 
							...
							<%
								...
								do While Not rs.EOF
									...
									data = "data-id='" & rs("EMP_Id") & "' data-pry='" & PRY_Id & "' data-tip='EMP' data-hito='113'"
									if(mode="mod") then
				...
				if(LIN_AgregaCivil) then%>
					<div id="civ12tab3">	<%'Organizaciones Civiles
						if(mode="mod") then%>
							<form role="form" action="/agregar-civil" method="POST" name="frm12s3_3" id="frm12s3_3" class="needs-validation">				
								<div class="row">                                                                         
									<div class="col-sm-5 col-md-5 col-lg-5">
										<div class="md-form input-with-post-icon">
											<div class="error-message">
												<div class="select">
													<select name="CIV_Id" id="CIV_Id" class="validate select-text form-control" <%=disabled%>>
														
													</select>
													<i class="fas fa-map-marker-alt input-prefix"></i>
													<span class="select-highlight"></span>
													<span class="select-bar"></span>
													<label class="select-label">Organizaciones Civiles</label>
												</div>
											</div>
										</div>							
									</div>
									<div class="col-sm-6 col-md-6 col-lg-6">
										<div class="md-form input-with-post-icon">
											<div class="error-message">					
												<i class="fas fa-tasks prefix"></i>
												<textarea id="PCI_Compromiso" name="PCI_Compromiso" class="md-textarea form-control" rows="5" required=""></textarea>
												<span class="select-bar"></span>
												<label for="PCI_Compromiso" class="active">Compromiso</label>							
											</div>
										</div>							
									</div>
									<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
										<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm12s3_3" name="btn_frm12s3_3"><i class="fas fa-plus"></i></button>	
									</div>						
								</div>	
								<input type="hidden" id="PRY_Id" value="<%=PRY_Id%>" name="PRY_Id">
							</form><%
						...
						<table id="tbl-orgsciv" class="ts table table-striped table-bordered dataTable table-sm" data-id="orgsciv" data-page="true" data-selected="true" data-keys="1"> 
							<thead> 
								<tr> 
									<th style="width:10px;">Id</th>
									<th>Organización Civil</th>								 
									<th>Rubro</th>
									<th>Compromiso</th><%
							...
							<%
								...
								do While Not rs.EOF
									...
									path="D:\DocumentosSistema\dialogosocial\" & PRY_Carpeta & "\verificadorcivil\c-" & yCIV_Id
									archivos=0
									If fs.FolderExists(path) = true Then
										Set carpeta = fs.getfolder(path)
										Set ficheros = carpeta.Files
									...
									data = "data-id='" & rs("CIV_Id") & "' data-pry='" & PRY_Id & "' data-tip='CIV' data-hito='114'"
									if(mode="mod") then								
										...
										dataCiviles = dataCiviles & "{""CIV_Id"":""" & rs("CIV_Id") & """,""CIV_Nombre"":""" & rs("CIV_Nombre") & """,""RUB_Nombre"":""" & rs("RUB_Nombre") & """,""PCI_Compromiso"":""" & rs("PCI_Compromiso") & """,""PCI_VerificadorCumplimiento"":""<i class='fas fa-cloud-download-alt " & clasedown & " " & colordown & "' style='cursor:" & disableddown & "' title='Bajar verificador' " & data & "></i>"""
									end if
				...
				if(LIN_AgregaGobierno) then%>
					<div id="gob12tab3">	<%'Organizaciones de Gobierno
						if(mode="mod") then%>
							<form role="form" action="/agregar-gobierno" method="POST" name="frm12s3_4" id="frm12s3_4" class="needs-validation">				
								<div class="row">                                                                         
									<div class="col-sm-5 col-md-5 col-lg-5">
										<div class="md-form input-with-post-icon">
											<div class="error-message">
									...
									<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
										<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm12s3_4" name="btn_frm12s3_4"><i class="fas fa-plus"></i></button>	
									</div>						
								</div>	
								<input type="hidden" id="PRY_Id" value="<%=PRY_Id%>" name="PRY_Id">
							</form><%
...
<script>	
	$(document).ready(function() {			
		...
		<%if(LIN_AgregaSindicato) then%>
			...
			$("#btn_frm12s3_1").click(function(){
				formValidate("#frm12s3_1")
				if($("#frm12s3_1").valid()){
					$.ajax({
						type: 'POST',			
						url: $("#frm12s3_1").attr("action"),
						data: $("#frm12s3_1").serialize(),
		...
		<%if(LIN_AgregaCivil) then%>
			...
			var civilesTable;
			var dataCiviles = <%=dataCiviles%>
			loadTableCiviles(dataCiviles);
			$('#tbl-orgsciv').css('width','100%')
			
			function loadTableCiviles(data) {
				$(".badgeciv").html(data.length);
				civilesTable = $('#tbl-orgsciv').DataTable({				
					lengthMenu: [ 5,10,20 ],
					data:data,
					columnDefs: [ {
					targets  : 'no-sort',
			...
			function loadCiviles(){
				$.ajax({
					type: 'POST',			
					url: '/listar-civil-sin-patrocinio',
					data: {PRY_Id:$("#PRY_Id").val()},
					success: function(data) {					
						param=data.split(bb);									
						if(param[0]=="200"){
							...
							swalWithBootstrapButtons.fire({
								icon:'error',								
								title: 'Ups!, no pude cargar las Org. Civiles',					
								text:param[1]
							});
						}
					},
			...
			$("#btn_frm12s3_3").click(function(){
				formValidate("#frm12s3_3")
				if($("#frm12s3_3").valid()){
					$.ajax({
						type: 'POST',			
						url: $("#frm12s3_3").attr("action"),
						data: $("#frm12s3_3").serialize(),
						success: function(data) {					
							param=data.split(bb);						
							civilesTable.clear().draw();
							civilesTable.rows.add(jQuery.parseJSON(param[1])).draw();
							
							$(".badgeciv").html(civilesTable.data().count());
							loadCiviles();
							if(param[0]=="200"){
								$("#PCI_Compromiso").val("");
								Toast.fire({
								icon: 'success',
								title: 'Organización Civil agregada correctamente'
								});							
							}else{
								swalWithBootstrapButtons.fire({
									icon:'error',								
									title: 'Ups!, no pude grabar los datos de la org. civil',					
									text:param[1]
								});
							}
						},
			...
			$("#pry-content").on("click",".delciv",function(e){
				e.preventDefault();
				e.stopPropagation();
				var CIV_Id = $(this).children().data("civ"); 
				var PRY_Id = $(this).children().data("pry");
				
				swalWithBootstrapButtons.fire({
				title: '¿Estas seguro?',
				text: "Esta acción eliminará la organización civil seleccionda",
				icon: 'question',
				showCancelButton: true,
				confirmButtonColor: '#3085d6',
				cancelButtonColor: '#d33',
				...
				if (result.value) {
						...
						$.ajax({
							type: 'POST',			
							url: '/elimina-civil',
							data: {PRY_Id:PRY_Id, CIV_Id:CIV_Id},
							success: function(data) {					
								param=data.split(bb);
								...
								if(param[0]=="200"){
									Toast.fire({
									icon: 'success',
									title: 'Organización Civil eliminada correctamente'
									});							
								}else{
									swalWithBootstrapButtons.fire({
										icon:'error',								
										title: 'Ups!, no pude eliminar los datos de la org. civil',
										text:param[1]
									});
								}
							},
		...
		<%if(LIN_AgregaEmpresa) then%>
			...
			$("#btn_frm12s3_2").click(function(){
				formValidate("#frm12s3_2")
				if($("#frm12s3_2").valid()){
					$.ajax({
						type: 'POST',			
						url: $("#frm12s3_2").attr("action"),
						data: $("#frm12s3_2").serialize(),
						success: function(data) {					
							param=data.split(bb);						
		...
		<%if(LIN_AgregaGobierno) then%>
			...
			$("#btn_frm12s3_4").click(function(){
				formValidate("#frm12s3_4")
				if($("#frm12s3_4").valid()){
					$.ajax({
						type: 'POST',			
						url: $("#frm12s3_4").attr("action"),
						data: $("#frm12s3_4").serialize(),
						success: function(data) {					
							param=data.split(bb);						
