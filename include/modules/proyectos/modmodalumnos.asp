...
<%	
	...
	sql="exec spAlumno_Modificar " & datos 	
	...
	if not rs.eof then
		result=rs("Result")
		if(result=3) then
			response.write("3\\Alumno ya se encuentra matriculado en este y en otro proyecto de la misma linea. Asociación y Modificaciones no realizadas")	
		else
			if(result=2) then
				response.write("3\\Alumno ya se encuentra matriculado en otro proeyecto de la misma linea. Asociación y Modificaciones no realizadas")	
			else
				if(result=1) then
					response.write("200\\Alumno ya se encuentra matriculado. Asociación no realizada, Modificaciones exitosas")	
				else
					response.write("200\\Alumno asociado a este proyecto y sus modificaciones realizadas exitosamente ")
				end if
			end if		
		end if
	end if
