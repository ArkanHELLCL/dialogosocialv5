...
<%
	...
	PRY_NumResAprueba	= up.form("PRY_NumResAprueba")
	PRY_FechaResolucion	= up.form("PRY_FechaResolucion")
	PRY_AdjuntoX		= up.form("PRY_AdjuntoX")
	...
	if not rs.eof then
		...
		path="D:\DocumentosSistema\dialogosocial\" & carpeta & "\contratos\"
	else%>
	...
	sqx="exec spProyectoLicitacion_Modificar " & PRY_Id & ",'" & PRY_Identificador & "'," & PRY_NumResAprueba & ",'" & PRY_FechaResolucion & "','" & PRY_AdjuntoX & "'," & session("ds5_usrid") & ",'" & session("ds5_usrtoken") & "'"
	...
	if cnn.Errors.Count > 0 then%>
	   {"state": 503, "message": "Error Conexión : <%=ErrMsg%>","data": "<%=sqx%>"}<%
		...
		response.end()
	End If%>	
	{"state": 200, "message": "Grabación de contrato correcta","data": null}