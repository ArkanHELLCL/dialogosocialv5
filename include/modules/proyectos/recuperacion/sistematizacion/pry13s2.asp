...
<%
	...
	'if(PRY_InformeSistematizacionEstado=0) or session("ds5_usrperfil")=1 then
	if((PRY_InformeSistematizacionEstado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=3)) or ((PRY_InformeSistematizacionEstado=1 and not PRY_InformeSistematizacionAceptado) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2)) then
		mode="mod"
	else
		mode="vis"
	end if
	...
	'response.end
%>
<form role="form" action="<%=action%>" method="POST" name="frm13s2" id="frm13s2" class="needs-validation">
	<h5>Documentos</h5>	
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentossis13" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentossis13" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm13s2" name="btn_frm13s2"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		$("#btn_frm13s2").click(function(){
			formValidate("#frm13s2")
			if($("#frm13s2").valid()){				
				$.ajax({
					type: 'POST',			
						...
						if(data.state=="200"){							
							...
							if(VPR_EstadoSubidoTotal<VPR_Total){
								swalWithBootstrapButtons.fire({
									...
									text:'Antes de avanzar al cierre del hito FINAL, debes tener arriba todos los documentos aqui solicitados'
								});	
							}else{								
								var bb = String.fromCharCode(92) + String.fromCharCode(92);
								$.ajax({
		...
		var documentosTableSis13;		
		loadTableDocumentos();
        $('#tbl-documentossis13').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentossis13")){				
				$('#tbl-documentossis13').dataTable().fnClearTable();
    			$('#tbl-documentossis13').dataTable().fnDestroy();
			}	
			
			documentosTableSis13 = $('#tbl-documentossis13').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){
										documentosTableSis13.ajax.reload();
										Toast.fire({
										icon: 'success',
