...
<%
	...
	if(mode="mod") then%>
		...
		<div style="padding-top:50px;"></div>
			<form role="form" action="<%=action%>" method="POST" name="frm13acepta" id="frm13acepta" class="needs-validation">
				<div class="row align-items-center">
					<div class="col-sm-12 col-md-6 col-lg-6" style="text-align:center;height:100%">
						<button type="button" class="btn btn-success btn-lg" id="btn_frm13acepta_1" name="btn_frm13acepta_1" value="enviar"><i class="fas fa-thumbs-up"></i> ACEPTAR</button>
						<button type="button" class="btn btn-danger btn-lg" id="btn_frm13acepta_2" name="btn_frm13acepta_2" value="enviar" data-toggle="modal" data-target="#modalRechaza"><i class="fas fa-thumbs-down"></i> RECHAZAR</button>
					</div>				
...
<div class="modal fade in" id="modalRechaza" tabindex="-1" role="dialog" aria-labelledby="modalRechazaLabel" aria-hidden="true">
	<div class="modal-dialog cascading-modal narrower modal-lg" role="document">  		
    	<div class="modal-content">		
			...
			<form role="form" action="/mod-13-h3-rechaza" method="POST" name="frmrechazainicio" id="frmrechazainicio" class="needs-validation">
...
<script>
	$(document).ready(function() {	
		...
		$("#btn_frm13acepta_1").click(function(){
			$.ajax({
				type: 'POST',			
				...
				success: function(data) {						
					if(data.state=="200"){						
						...
						if(VPR_EstadoAprobadoTotal<VPR_Total){
							...
							$.ajax({
								type: 'POST',			
								url: '/lista-estado-verificadores-mesas',
								data: {PRY_Id:<%=PRY_Id%>},
								dataType: "json",
								success: function(data) {						
									if(data.state=="200"){						
										...
										if(VER_EstadoAprobado<VER_Total){
											...
											swalWithBootstrapButtons.fire({
												...
												text: '¿Estas seguro de querer aceptar el informe "FINAL"?',
												icon: 'success',
											...
											if (result.value) {			  					
												$.ajax({
													type: 'POST',			
													url: $("#frm13acepta").attr("action"),
													data: $("#frm13acepta").serialize(),
													dataType: "json",
													success: function(data) {						
														if(data.state=="200"){
															$("#frm13acepta")[0].reset();
															...
															wrk_informes('/prt-informefinalrecuperacion','informefinalrecuperacion.pdf',<%=PRY_Id%>,'<%=PRY_Identificador%>','/mnu-13',<%=session("ds5_usrid")%>,'<%=session("ds5_usrtoken")%>');
		...
		$("#btn_rechazainicio").click(function(){		
			...
			if($("#frmrechazainicio").valid()){	
				...
				swalWithBootstrapButtons.fire({
				  ...
				  text: '¿Estas seguro de querer rechazar el informe "FINAL"?',
				  icon: 'error',
				  showCancelButton: true,
				  confirmButtonColor: '#3085d6',
				  cancelButtonColor: '#d33',
