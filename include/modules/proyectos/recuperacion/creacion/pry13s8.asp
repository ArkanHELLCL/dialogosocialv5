...
<%
	...
	if(mode="mod") then%>
		...
		<form role="form" action="/agregar-tematica" method="POST" name="frm13s7_1" id="frm13s7_1" class="needs-validation">			
			<div class="row"> 
				...
				<div class="col align-self-end">
					<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm13s7_1" name="btn_frm13s7_1" style="float: right;"><i class="fas fa-plus"></i></button>
				</div>
			</div>
...
<script>
	$(document).ready(function() {	
		...
		$("#btn_frm13s7_1").click(function(){
			formValidate("#frm13s7_1")
			if($("#frm13s7_1").valid()){
				$.ajax({
					type: 'POST',			
					url: $("#frm13s7_1").attr("action"),
					data: $("#frm13s7_1").serialize(),
