...
<%
	...
	if(mode="mod" or mode="vis") then		
		...
		if not rs.eof then	
			...
			PRY_MetodologiaResultadoEsperado=rs("PRY_MetodologiaResultadoEsperado")
		else
	...
	<h5>Metodología y Resultados Esperados</h5>
	<form role="form" action="<%=action%>" method="POST" name="frm13s7" id="frm13s7" class="needs-validation">			
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12">
				<div class="md-form">
					<div class="error-message">								
						<i class="fas fa-comment prefix"></i>
							<textarea id="PRY_MetodologiaResultadoEsperado" name="PRY_MetodologiaResultadoEsperado" class="md-textarea form-control" <%=disabled%> rows="7"><%=PRY_MetodologiaResultadoEsperado%></textarea>
						<span class="select-bar"></span><%
						clase=""
						if(PRY_MetodologiaResultadoEsperado<>"") then
							clase="active"
						end if%>
						<label for="" class="<%=clase%>">Metodología y Resultados Esperados</label>									
					</div>
				</div>
			</div>
		</div>			
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm13s7" name="btn_frm13s7"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm13s7").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm13s7");			
						
			if($("#frm13s7").valid()){
				
					$.ajax({
						type: 'POST',			
						url: $("#frm13s7").attr("action"),
						data: $("#frm13s7").serialize(),
