...
<%
	...
	if(mode="mod" or mode="vis") then
		...
		if not rs.eof then
			...
			PRY_EncargadoAudio=rs("PRY_EncargadoAudio")
			PRY_EncargadoAudioMail=rs("PRY_EncargadoAudioMail")
			PRY_EncargadoAudioCelular=rs("PRY_EncargadoAudioCelular")
			SEX_IdEncargadoAudio=rs("SEX_IdEncargadoAudio")
			PRY_EncargadoAudioCarrera=rs("PRY_EncargadoAudioCarrera")
			EDU_IdEncargadoAudio=rs("EDU_IdEncargadoAudio")
			PRY_EncargadoAudioForEsp=rs("PRY_EncargadoAudioForEsp")
	...
	'response.end
%>
<form role="form" action="<%=action%>" method="POST" name="frm13s3" id="frm13s3" class="needs-validation">
		...
		<div class="col-sm-12 col-md-4 col-lg-4">
			<div class="md-form input-with-post-icon">
				<div class="error-message">								
					<i class="fas fa-user input-prefix"></i><%
					if(mode="mod" or mode="vis") then%>
						<input type="text" id="PRY_EncargadoAudio" name="PRY_EncargadoAudio" class="form-control" <%=disabled%> value="<%=PRY_EncargadoAudio%>"><%
					else%>
						<input type="text" id="PRY_EncargadoAudio" name="PRY_EncargadoAudio" class="form-control" <%=disabled%>><%
					end if%>
					<span class="select-bar"></span><%
					if PRY_EncargadoAudio<>"" and not IsNULL(PRY_EncargadoAudio) then
						lblClass="active"
					else
						lblClass=""
					end if%>
					<label for="PRY_EncargadoAudio" class="<%=lblClass%>">Nombre</label>									
				</div>
			</div>
		</div>
		<div class="col-sm-12 col-md-4 col-lg-4">
			<div class="md-form input-with-post-icon">
				<div class="error-message">								
					<i class="fas fa-envelope input-prefix"></i><%
					if(mode="mod" or mode="vis") then%>										
						<input type="email" id="PRY_EncargadoAudioMail" name="PRY_EncargadoAudioMail" class="form-control" <%=disabled%> value="<%=PRY_EncargadoAudioMail%>"><%
					else%>
						<input type="email" id="PRY_EncargadoAudioMail" name="PRY_EncargadoAudioMail" class="form-control" <%=disabled%> value="<%=%>"><%
					end if%>
					<span class="select-bar"></span><%
					if PRY_EncargadoAudioMail<>"" then
						lblClass="active"
					else
						lblClass=""	
					end if%>
					<label for="PRY_EncargadoAudioMail" class="<%=lblClass%>">Correo electrónico</label>									
				</div>
			</div>
		</div>
		<div class="col-sm-12 col-md-4 col-lg-4" style="text-align: left;">			
			<label for="SEX_IdEncargadoAudio" class="radiolabel">Sexo</label>
			<div class="md-radio radio-lightBlue md-radio-inline"><%
				if(SEX_IdEncargadoAudio=1) or (IsNULL(SEX_IdEncargadoAudio)) then%>
					<input id="SEX_IdEncargadoAudiofemenino" type="radio" name="SEX_IdEncargadoAudio" checked <%=checkbox%> value=1><%
				else%>
					<input id="SEX_IdEncargadoAudiofemenino" type="radio" name="SEX_IdEncargadoAudio" <%=checkbox%> value=1><%
				end if%>
				<label for="SEX_IdEncargadoAudiofemenino">Femenino</label>
			</div>
			<div class="md-radio radio-lightBlue md-radio-inline"><%
				if(SEX_IdEncargadoAudio=2) then%>
					<input id="SEX_IdEncargadoAudiomasculino" type="radio" name="SEX_IdEncargadoAudio" checked <%=checkbox%> value="2"><%
				else%>
					<input id="SEX_IdEncargadoAudiomasculino" type="radio" name="SEX_IdEncargadoAudio" <%=checkbox%> value="2"><%
				end if%>
				<label for="SEX_IdEncargadoAudiomasculino">Masculino</label>
			</div>			
		</div>
	</div>	
	<div class="row align-items-center" style="padding-bottom:30px;"> 				
		<div class="col-sm-12 col-md-2 col-lg-2">
			<div class="md-form input-with-post-icon">
				<div class="error-message">
					<i class="fas fa-mobile-alt input-prefix"></i><%
					if(mode="mod" or mode="vis") then%>
						<input type="number" id="PRY_EncargadoAudioCelular" name="PRY_EncargadoAudioCelular" class="form-control" <%=disabled%> value="<%=PRY_EncargadoAudioCelular%>"><%
					else%>
						<input type="number" id="PRY_EncargadoAudioCelular" name="PRY_EncargadoAudioCelular" class="form-control" <%=disabled%>><%
					end if%>
					<span class="select-bar"></span><%
					if PRY_EncargadoAudioCelular<>"" then
						lblClass="active"
					else
						lblClass=""
					end if%>
					<label for="PRY_EncargadoAudioCelular" class="<%=lblClass%>">Teléfono</label>									
				</div>
			</div>
		</div>
		<div class="col-sm-12 col-md-3 col-lg-3">
			<div class="md-form input-with-post-icon">
				<div class="error-message">
					<div class="select">
						<select name="EDU_IdEncargadoAudio" id="EDU_IdEncargadoAudio" class="validate select-text form-control" <%=disabled%>>
							<option value="" disabled selected></option><%													
							set rx = cnn.Execute("exec spEducacion_Listar")
							on error resume next					
							do While Not rx.eof
								if(EDU_IdEncargadoAudio=rx("EDU_Id")) then%>
									<option value="<%=rx("EDU_Id")%>" selected><%=rx("EDU_Nombre")%></option><%
								else%>
									<option value="<%=rx("EDU_Id")%>"><%=rx("EDU_Nombre")%></option><%
								end if
		...
		<div class="col-sm-12 col-md-3 col-lg-3">
			<div class="md-form input-with-post-icon">
				<div class="error-message">								
					<i class="fas fa-user input-prefix"></i><%
					if(mode="mod" or mode="vis") then%>
						<input type="text" id="PRY_EncargadoAudioCarrera" name="PRY_EncargadoAudioCarrera" class="form-control" <%=disabled%> value="<%=PRY_EncargadoAudioCarrera%>"><%
					else%>
						<input type="text" id="PRY_EncargadoAudioCarrera" name="PRY_EncargadoAudioCarrera" class="form-control" <%=disabled%>><%
					end if%>
					<span class="select-bar"></span><%
					if PRY_EncargadoAudioCarrera<>"" then
						lblClass="active"
					else
						lblClass=""
					end if%>
					<label for="PRY_EncargadoAudioCarrera" class="<%=lblClass%>">Nombre Carrera</label>									
				</div>
			</div>
		</div>
		<div class="col-sm-12 col-md-4 col-lg-4" style="text-align: left;">			
			<label for="PRY_EncargadoAudioForEsp" class="radiolabel">Formación Especializada</label>
			<div class="md-radio radio-lightBlue md-radio-inline"><%
				if(PRY_EncargadoAudioForEsp=1) or (IsNULL(PRY_EncargadoAudioForEsp)) then%>
					<input id="PRY_EncargadoAudioForEspSI" type="radio" name="PRY_EncargadoAudioForEsp" checked value="1" <%=checkbox%>><%
				else%>
					<input id="PRY_EncargadoAudioForEspSI" type="radio" name="PRY_EncargadoAudioForEsp" value="1" <%=checkbox%>><%
				end if%>
				<label for="PRY_EncargadoAudioForEspSI">Si</label>
			</div>
			<div class="md-radio radio-lightBlue md-radio-inline"><%
				if(PRY_EncargadoAudioForEsp=2) then%>
					<input id="PRY_EncargadoAudioForEspNO" type="radio" name="PRY_EncargadoAudioForEsp" checked value="2" <%=checkbox%>><%
				else%>
					<input id="PRY_EncargadoAudioForEspNO" type="radio" name="PRY_EncargadoAudioForEsp" value="2" <%=checkbox%>><%
				end if%>
				<label for="PRY_EncargadoAudioForEspNO">No</label>
			</div>			
		</div>
	</div>
...
<div class="row">		
	<div class="footer"><%
		if mode="mod" or mode="add" then%>		
			<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm13s3" name="btn_frm13s3"><%=txtBoton%></button><%
		else%>
...
<script>
	...
	$(document).ready(function() {						
		...
		$("#btn_frm13s3").click(function(){
								
			formValidate("#frm13s3")
			if($("#frm13s3").valid()){
				var bb = String.fromCharCode(92) + String.fromCharCode(92);
				$.ajax({
					type: 'POST',			
					url: $("#frm13s3").attr("action"),
					data: $("#frm13s3").serialize(),
