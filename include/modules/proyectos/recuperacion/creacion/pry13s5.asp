...
%>
		...
		<div class="container-nav" style="margin-right: 15px;margin-left: 15px;">
			...
			<div class="tab-content">
				<div id="sintab1">	<%'Organizaciones Sindicales
					if(mode="mod") then%>
						<form role="form" action="/agregar-sindicato" method="POST" name="frm13s5_1" id="frm13s5_1" class="needs-validation">
							<div class="row">                                                                         
								...
								<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
									<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm13s5_1" name="btn_frm13s5_1"><i class="fas fa-plus"></i></button>	
								</div>						
							</div>	
				...
				<div id="emptab2">	<%'Organizaciones Empresariales
					if(mode="mod") then%>
						<form role="form" action="/agregar-empresa" method="POST" name="frm13s5_2" id="frm13s5_2" class="needs-validation">				
							<div class="row">                                                                         
								<div class="col-sm-5 col-md-5 col-lg-5">
									<div class="md-form input-with-post-icon">
										<div class="error-message">
								...
								<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
									<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm13s5_2" name="btn_frm13s5_2"><i class="fas fa-plus"></i></button>	
								</div>						
							</div>	
							<input type="hidden" id="PRY_Id" value="<%=PRY_Id%>" name="PRY_Id">
						</form><%
				...
				if(PRY_TipoMesa=2) then%>				
					<div id="gobtab3">	<%'Organizaciones de Gobierno
						if(mode="mod") then%>
							<form role="form" action="/agregar-gobierno" method="POST" name="frm13s5_3" id="frm13s5_3" class="needs-validation">				
								<div class="row">                                                                         
									<div class="col-sm-5 col-md-5 col-lg-5">
										<div class="md-form input-with-post-icon">
											<div class="error-message">
									...
									<div class="col-sm-1 col-md-1 col-lg-1" style="padding-top: 23px;text-align:left;">
										<button type="button" class="btn btn-success btn-md waves-effect waves-dark" id="btn_frm13s5_3" name="btn_frm13s5_3"><i class="fas fa-plus"></i></button>	
									</div>						
								</div>	
								<input type="hidden" id="PRY_Id" value="<%=PRY_Id%>" name="PRY_Id">
							</form><%
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>
				<form role="form" action="<%=action%>" method="POST" name="frm13s5" id="frm13s5" class="needs-validation">
					<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm13s5" name="btn_frm13s5"><%=txtBoton%></button>
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm13s5_1").click(function(){
			formValidate("#frm13s5_1")
			if($("#frm13s5_1").valid()){
				$.ajax({
					type: 'POST',			
					url: $("#frm13s5_1").attr("action"),
					data: $("#frm13s5_1").serialize(),
		...
		$("#btn_frm13s5_2").click(function(){
			formValidate("#frm13s5_2")
			if($("#frm13s5_2").valid()){
				$.ajax({
					type: 'POST',			
					url: $("#frm13s5_2").attr("action"),
					data: $("#frm13s5_2").serialize(),
					success: function(data) {					
						param=data.split(bb);						
		...
		$("#btn_frm13s5_3").click(function(){
			formValidate("#frm13s5_3")
			if($("#frm13s5_3").valid()){
				$.ajax({
					type: 'POST',			
					url: $("#frm13s5_3").attr("action"),
					data: $("#frm13s5_3").serialize(),
					success: function(data) {					
						param=data.split(bb);						
		...
		$("#btn_frm13s5").click(function(){
			formValidate("#frm13s5")
			if($("#frm13s5").valid()){
				var bb = String.fromCharCode(92) + String.fromCharCode(92);
				$.ajax({
					type: 'POST',			
					url: $("#frm13s5").attr("action"),
					data: $("#frm13s5").serialize(),
					success: function(data) {					
						param=data.split(bb)
						if(param[0]=="200"){
							Toast.fire({
