...
<%
	...
	if(mode="mod") then%>
		...
		<h6>Aceptar o Rechazar Informe INICIAL</h6>
			...
			<form role="form" action="<%=action%>" method="POST" name="frm13acepta" id="frm13acepta" class="needs-validation">
				<div class="row align-items-center">
					...
					<div class="col-sm-12 col-md-6 col-lg-6">
						<blockquote>
							<p>
								Después de haber revisado y corregido la información en los pasos anteriores, es necesario aceptar el informe "Inicial", con el fin informar al ejecutor este hecho y asi poder continuar con el ingreso de la información del siguiente informe.
							</p>
							<p>
							...
							<p>
								Al rechazar el informe se liberará el HITO INICIAL para que el ejecutor pueda subsanar las observaciones que deben ser ingresadas un vez presionado el botón "RECHAZAR"
							</p>
						</blockquote>                                		                                                                    
					</div>                               		                                    
				</div>
		...
		<h6>Informe INICIAL en proceso de revisión</h6>
		<div style="padding-top:50px;"></div>
			<form role="form" action="" method="POST" name="" id="" class="needs-validation">
				<div class="row align-items-center">
					...
					<div class="col-sm-12 col-md-10 col-lg-10">
						<blockquote>
							<p>
								Estimado <%=perfil%>, actualmente el informe INICIAL se encuentra en proceso de revisión. En este período el revisor del proyecto podría enviarle algunas solicitudes de adecuaciones requeridas para completar el informe y asi proceder a la aceptación del mismo.
							</p>
							<p>
...
<!-- Formulario para rechazar el informe inicial -->
<div class="modal fade in" id="modalRechaza" tabindex="-1" role="dialog" aria-labelledby="modalRechazaLabel" aria-hidden="true">
	<div class="modal-dialog cascading-modal narrower modal-lg" role="document">  		
    	<div class="modal-content">		
			...
			<form role="form" action="/mod-13-h1-rechaza" method="POST" name="frmrechazainicio" id="frmrechazainicio" class="needs-validation">
					...
					if(data.state=="200"){						
						...
						if(VPR_EstadoAprobadoTotal<VPR_Total){
							swalWithBootstrapButtons.fire({
								...
								text:'Antes de aceptar el informe INICIAL, debes tener APROBADOS todos los documentos solicitados en el ítem "Documentos"'
							});	
						}else{										
							swalWithBootstrapButtons.fire({
								...
								$.ajax({
									...
									success: function(data) {						
										if(data.state=="200"){
											...
											wrk_informes('/prt-informeinicialrecuperacion','informeinicialrecuperacion.pdf',<%=PRY_Id%>,'<%=PRY_Identificador%>','/mnu-13',<%=session("ds5_usrid")%>,'<%=session("ds5_usrtoken")%>');
		...
		$("#btn_rechazainicio").click(function(){
			...
			if($("#frmrechazainicio").valid()){	
				...
				swalWithBootstrapButtons.fire({
				  ...
				  text: '¿Estas seguro de querer rechazar el informe "INICIAL" para liberar el HITO al ejecutor?',
				  icon: 'error',
				  showCancelButton: true,
				  confirmButtonColor: '#3085d6',
				  cancelButtonColor: '#d33',
