...
<form role="form" action="<%=action%>" method="POST" name="frm13s1" id="frm13s1" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosini13" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosini13" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableIni13;		
		loadTableDocumentos();
        $('#tbl-documentosini13').css('width','99%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosini13")){				
				$('#tbl-documentosini13').dataTable().fnClearTable();
    			$('#tbl-documentosini13').dataTable().fnDestroy();
			}	
			
			documentosTableIni13 = $('#tbl-documentosini13').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){
										documentosTableIni13.ajax.reload();
										Toast.fire({
										icon: 'success',
