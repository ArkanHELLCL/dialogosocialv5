...
<%
	...
	set rs = cnn.Execute("exec spVerificadorPlanMarketing_Listar " & PRY_Id)
	on error resume next
	if cnn.Errors.Count > 0 then 
		ErrMsg = cnn.Errors(0).description
		response.write("Error spVerificadorPlanMarketing_Listar")
		cnn.close 		
		response.end
	End If	
	cont=1
	...
	do While Not rs.EOF
		...
		'if(mode="mod") and (session("ds5_usrperfil")=1) then	'Adm - todo
		''	acciones="<i class='fas fa-cloud-upload-alt " & claseup & " " & colorup & "' style='cursor:" & disabledup & "' title='Subir documento' " & data & "></i> <i class='fas fa-cloud-download-alt " & clasedown & " " & colordown & "' style='cursor:" & disableddown & "' title='Bajar documento' " & data & "></i> <i class='fas fa-check " & colorcheck & " " & clasecheck & "' style='cursor:" & disabledcheck & "' data-pry='" & PRY_Id & "' " & data & "></i> <i class='fas fa-thumbs-up " & coloraccept & " " & claseaccept & "' style='cursor:" & disabledaccept & "' data-pry='" & PRY_Id & "' " & data & "></i> <i class='fas fa-thumbs-down " & colorreject & " " & clasereject & "' style='cursor:" & disabledreject & "' data-pry='" & PRY_Id & "' " & data & "></i> <i class='fas fa-trash-alt " & colordel & " " & clasedel & "' style='cursor:" & disableddel & "' data-pry='" & PRY_Id & "' " & data & "></i>"			
		'end if

		'if(mode="mod") and (session("ds5_usrperfil")=3) then	'eje
		''	if(rs("VPM_EstadoAprobado")="" or IsNULL(rs("VPM_EstadoAprobado")) or rs("VPM_EstadoAprobado")=0) then	'solo subir, bajar y eliminar
		...
		if(rs("VPM_EstadoAprobado")=1) then
			aprobado="<i class='fas fa-thumbs-up aprobado text-success'></i>"
		else
			aprobado="-"
		end if			
		if(rs("VPM_EstadoSubido")=1) then
			subido="<i class='fas fa-thumbs-up subido text-primary'></i>"
		else
			subido="-"
		end if
		if(rs("VPM_EstadoRechazado")=1) then
			rechazado="<i class='fas fa-thumbs-down rechazado text-danger'></i>"
		else
			rechazado="-"
		end if
		if(rs("VPM_EstadoRevisado")=1) then
			revisado="<i class='fas fa-thumbs-up revisado text-warning'></i>"
		else
			revisado="-"
		end if
		if(rs("VPM_EstadoEliminado")=1) then
			eliminado="<i class='fas fa-trash-alt eliminado text-danger'></i>"
		else
			eliminado="-"
		end if
	
		dataPlnMrk = dataPlnMrk & "[""" & rs("VPM_Id") & """,""" & rs("VPM_AccionComprometida") & """,""" & rs("VPM_Etapa") & """,""" & subido & """,""" & revisado & """,""" & aprobado & """,""" & rechazado	& """,""" & acciones & """]"		
		
		rs.movenext
		if not rs.eof then
			dataPlnMrk = dataPlnMrk & ","
		end if
		
	loop
	dataPlnMrk=dataPlnMrk & "]}"
	
	response.write(dataPlnMrk)	
%>