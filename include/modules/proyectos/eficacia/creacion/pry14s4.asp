...
<%
	...
	<form role="form" action="<%=action%>" method="POST" name="frm14s4" id="frm14s4" class="needs-validation">			
		<div class="row">
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm14s4" name="btn_frm14s4"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm14s4").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm14s4");			
						
			if($("#frm14s4").valid()){
				
					$.ajax({
						type: 'POST',			
						url: $("#frm14s4").attr("action"),
						data: $("#frm14s4").serialize(),
