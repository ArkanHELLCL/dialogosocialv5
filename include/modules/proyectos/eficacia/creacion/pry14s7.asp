...
<%
	...
	<form role="form" action="<%=action%>" method="POST" name="frm14s7" id="frm14s7" class="needs-validation">			
		<div class="row">
	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" then%>			
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm14s7" name="btn_frm14s7"><%=txtBoton%></button><%					
			else%>				
...
<script>	
	$(document).ready(function() {			
		...
		$("#btn_frm14s7").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();
			
			formValidate("#frm14s7");			
						
			if($("#frm14s7").valid()){
				
					$.ajax({
						type: 'POST',			
						url: $("#frm14s7").attr("action"),
						data: $("#frm14s7").serialize(),
