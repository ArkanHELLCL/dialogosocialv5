...
<%
	...
	'response.end
%>
<form role="form" action="<%=action%>" method="POST" name="frm14s3" id="frm14s3" class="needs-validation">
...
<div class="row">		
	<div class="footer"><%
		if mode="mod" or mode="add" then%>		
			<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="btn_frm14s3" name="btn_frm14s3"><%=txtBoton%></button><%
		else%>
...
<script>
	...
	$(document).ready(function() {						
		...
		$("#btn_frm14s3").click(function(){
								
			formValidate("#frm14s3")
			if($("#frm14s3").valid()){
				var bb = String.fromCharCode(92) + String.fromCharCode(92);
				$.ajax({
					type: 'POST',			
					url: $("#frm14s3").attr("action"),
					data: $("#frm14s3").serialize(),
