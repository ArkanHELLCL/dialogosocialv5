	...
	<div class="row">		
		<div class="footer"><%
			if mode="mod" or mode="add" then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect waves-dark" id="<%=id%>" name="btn_frm14s1"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {			
		...
		$("#btn_frm14s1, #btn_frm14s2").click(function(){
			formValidate("#frm14s1")
			if($("#frm14s1").valid()){
				var bb = String.fromCharCode(92) + String.fromCharCode(92);
				$.ajax({
					type: 'POST',			
					url: $("#frm14s1").attr("action"),
