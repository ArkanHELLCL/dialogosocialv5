...
<%
	...
	if((PRY_InformeParcialEstado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=3)) or ((PRY_InformeParcialEstado=1 and PRY_InformeParcialAceptado=0) and (session("ds5_usrperfil")=1 or session("ds5_usrperfil")=2)) then
		mode="mod"
	else
		mode="vis"
	end if
...
<form role="form" action="<%=action%>" method="POST" name="frm14s1" id="frm14s1" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentosava14" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentosava14" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableAva14;		
		loadTableDocumentos();
        $('#tbl-documentosava14').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentosava14")){				
				$('#tbl-documentosava14').dataTable().fnClearTable();
    			$('#tbl-documentosava14').dataTable().fnDestroy();
			}	
			
			documentosTableAva14 = $('#tbl-documentosava14').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){										
										documentosTableAva14.ajax.reload();
										Toast.fire({
										icon: 'success',
