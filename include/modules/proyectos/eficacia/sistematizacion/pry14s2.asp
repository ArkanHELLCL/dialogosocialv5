...
<form role="form" action="<%=action%>" method="POST" name="frm14s2" id="frm14s2" class="needs-validation">
	...
	<div class="row" style="padding-top:30px;">
		<div class="col-12" style="overflow: auto;">
			<table id="tbl-documentossis14" class="ts table table-striped table-bordered dataTable table-sm" data-id="documentossis14" data-page="true" data-selected="true" data-keys="1"> 
				<thead> 
					<tr> 
	...
	<div class="row">		
		<div class="footer"><%
			if(mode="mod" or mode="add") and (session("ds5_usrperfil")<>2) then%>		
				<button type="button" class="btn <%=btnColor%> btn-md waves-effect" id="btn_frm14s2" name="btn_frm14s2"><%=txtBoton%></button><%
			else%>
...
<script>
	...
	$(document).ready(function() {
		...
		var documentosTableSis14;		
		loadTableDocumentos();
        $('#tbl-documentossis14').css('width','100%')
		
		function loadTableDocumentos() {
			if($.fn.DataTable.isDataTable( "#tbl-documentossis14")){				
				$('#tbl-documentossis14').dataTable().fnClearTable();
    			$('#tbl-documentossis14').dataTable().fnDestroy();
			}	
			
			documentosTableSis14 = $('#tbl-documentossis14').DataTable({				
				lengthMenu: [ 10,15,20 ],
				ajax:{
					...
					if(fileOk){					
						...
						if(sizerror){
							...
							$.ajax({
								...
								success: function (data) {								
									if(data.state==200){
										documentosTableSis14.ajax.reload();
										Toast.fire({
										icon: 'success',
